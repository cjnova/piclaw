        ...
        const sse = new SSEClient(
            (eventType, data) => {
                ...
                if (eventType === 'extension_ui_request') {
                    ...
                    const promptSelect = () => {
                        if (options.length === 0) return window.prompt(title, '');
                        const list = options.map((opt, idx) => `${idx + 1}. ${opt}`).join('\n');
                        const promptText = `${title}\n${list}\n\nEnter number or value`;
                        const raw = window.prompt(promptText, '');
                        if (raw === null) return null;
                        const idx = parseInt(raw, 10);
                        if (!Number.isNaN(idx) && options[idx - 1]) return options[idx - 1];
                    ...
                    let outcome = null;
                    if (kind === 'confirm') {
                        outcome = window.confirm(message ? `${title}\n\n${message}` : title);
                    } else if (kind === 'select') {
                        outcome = promptSelect();
                    } else if (kind === 'editor') {
                        outcome = window.prompt(title, prefill);
                    } else {
                        outcome = window.prompt(title, placeholder);
                    }

                    respondToAgentRequest(requestId, outcome).catch((err) => {
                        console.error('Failed to respond to extension UI request:', err);
                    });
                    return;
                }

                if (eventType === 'extension_ui_notify') {
                    ...
                    if (type === 'error' || type === 'warning') {
                        ...
                        console.log('Extension notice:', message);
                    }
                    return;
                }
                ...
                if (eventType === 'extension_ui_status') {
                    if (data?.text) console.log('Extension status:', data.text);
                    return;
                }

                if (eventType === 'extension_ui_timeout') {
                    console.warn('Extension UI request timed out:', data);
                    return;
                }
                ...
                if (eventType === 'extension_ui_working') {
                    if (data?.message) console.log('Extension working:', data.message);
                    return;
                }

                if (eventType === 'extension_ui_widget') {
                    console.log('Extension widget:', data);
                    return;
                }

                if (eventType === 'extension_ui_editor_text') {
                    console.log('Extension editor text updated');
                    return;
                }
