...
`);N=W.pop()||"";for(let Q of W){let L=y8(Q);if(L)$(L.event,L.data)}}N+=Z.decode();let Y=y8(N);if(Y)$(Y.event,Y.data)}async function A2(_=10,$=null,j=null){let Z=`/timeline?limit=${_}`;if($)Z+=`&before=${$}`;if(j)Z+=`&chat_jid=${encodeURIComponent(j)}`;return i_(Z)}async function U4(_,$=50,j=0,Z=null){let N=Z?`&chat_jid=${encodeURIComponent(Z)}`:"";return i_(`/hashtag/${encodeURIComponent(_)}?limit=${$}&offset=${j}${N}`)}async function O4(_,$=50,j=0,Z=null,N="current",Y=null){let z=Z?`&chat_jid=${encodeURIComponent(Z)}`:"",G=N?`&scope=${encodeURIComponent(N)}`:"",W=Y?`&root_chat_jid=${encodeURIComponent(Y)}`:"";return i_(`/search?q=${encodeURIComponent(_)}&limit=${$}&offset=${j}${z}${G}${W}`)}async function F4(_,$=null){let j=$?`?chat_jid=${encodeURIComponent($)}`:"";return i_(`/thread/${_}${j}`)}async function t5(_,$=[],j=null){let Z=j?`?chat_jid=${encodeURIComponent(j)}`:"";return i_(`/post${Z}`,{method:"POST",body:JSON.stringify({content:_,media_ids:$})})}async function e5(_,$,j=[],Z=null){let N=Z?`?chat_jid=${encodeURIComponent(Z)}`:"";return i_(`/post/reply${N}`,{method:"POST",body:JSON.stringify({thread_id:_,content:$,media_ids:j})})}async function H4(_,$=!1,j=null){let Z=j?`&chat_jid=${encodeURIComponent(j)}`:"",N=`/post/${_}?cascade=${$?"true":"false"}${Z}`;return i_(N,{method:"DELETE"})}async function y2(_,$,j=null,Z=[],N=null,Y=null){let z=Y?`?chat_jid=${encodeURIComponent(Y)}`:"";return i_(`/agent/${_}/message${z}`,{method:"POST",body:JSON.stringify({content:$,thread_id:j,media_ids:Z,mode:N})})}async function D4(){return i_("/agent/active-chats")}async function _7(_=null,$={}){let j=new URLSearchParams;if(_)j.set("root_chat_jid",String(_));if($?.includeArchived)j.set("include_archived","1");let Z=j.toString()?`?${j.toString()}`:"";return i_(`/agent/branches${Z}`)}async function _1(_,$={}){return i_("/agent/branch-fork",{method:"POST",body:JSON.stringify({source_chat_jid:_,...$?.agentName?{agent_name:$.agentName}:{},...$?.displayName?{display_name:$.displayName}:{}})})}async function $7(_,$={}){return i_("/agent/branch-rename",{method:"POST",body:JSON.stringify({chat_jid:_,...$&&Object.prototype.hasOwnProperty.call($,"agentName")?{agent_name:$.agentName}:{},...$&&Object.prototype.hasOwnProperty.call($,"displayName")?{display_name:$.displayName}:{}})})}async function j7(_){return i_("/agent/branch-prune",{method:"POST",body:JSON.stringify({chat_jid:_})})}async function Z7(_,$={}){return i_("/agent/branch-restore",{method:"POST",body:JSON.stringify({chat_jid:_,...$&&Object.prototype.hasOwnProperty.call($,"agentName")?{agent_name:$.agentName}:{},...$&&Object.prototype.hasOwnProperty.call($,"displayName")?{display_name:$.displayName}:{}})})}async function N7(_,$,j,Z="auto",N={}){let Y={source_chat_jid:_,content:j,mode:Z,...N?.sourceAgentName?{source_agent_name:N.sourceAgentName}:{},...N?.targetBy==="agent_name"?{target_agent_name:$}:{target_chat_jid:$}};return i_("/agent/peer-message",{method:"POST",body:JSON.stringify(Y)})}async function J4(){return i_("/agent/roster")}async function E4(_=null){let $=_?`?chat_jid=${encodeURIComponent(_)}`:"";return i_(`/agent/status${$}`)}async function Y7(_=null){let $=_?`?chat_jid=${encodeURIComponent(_)}`:"";return i_(`/agent/context${$}`)}async function z7(_=null){let $=_?`?chat_jid=${encodeURIComponent(_)}`:"";return i_(`/agent/queue-state${$}`)}async function K7(_,$=null){let j=await fetch("/agent/queue-remove",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({row_id:_,chat_jid:$||void 0})});if(!j.ok){let Z=await j.json().catch(()=>({error:"Failed to remove queued item"}));throw Error(Z.error||`HTTP ${j.status}`)}return j.json()}async function V7(_,$=null){let j=await fetch("/agent/queue-steer",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({row_id:_,chat_jid:$||void 0})});if(!j.ok){let Z=await j.json().catch(()=>({error:"Failed to steer queued item"}));throw Error(Z.error||`HTTP ${j.status}`)}return j.json()}async function $1(_=null){let $=_?`?chat_jid=${encodeURIComponent(_)}`:"";return i_(`/agent/models${$}`)}async function A4(_){let $=new FormData;$.append("file",_);let j=await fetch("/media/upload",{method:"POST",body:$});if(!j.ok){let Z=await j.json().catch(()=>({error:"Upload failed"}));throw Error(Z.error||`HTTP ${j.status}`)}return j.json()}async function k1(_,$,j=null){let Z=await fetch("/agent/respond",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({request_id:_,outcome:$,chat_jid:j||void 0})});if(!Z.ok){let N=await Z.json().catch(()=>({error:"Failed to respond"}));throw Error(N.error||`HTTP ${Z.status}`)}return Z.json()}async function y4(_){let $=await fetch("/agent/card-action",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(_)});if(!$.ok){let j=await $.json().catch(()=>({error:"Adaptive Card action failed"}));throw Error(j.error||`HTTP ${$.status}`)}return $.json()}async function G7(_,$={}){let j=await fetch("/agent/side-prompt/stream",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({prompt:_,system_prompt:$.systemPrompt||void 0,chat_jid:$.chatJid||void 0}),signal:$.signal});if(!j.ok){let Y=await j.json().catch(()=>({error:"Side prompt failed"}));throw Error(Y.error||`HTTP ${j.status}`)}let Z=null,N=null;if(await a5(j,(Y,z)=>{if($.onEvent?.(Y,z),Y==="side_prompt_thinking_delta")$.onThinkingDelta?.(z?.delta||"");else if(Y==="side_prompt_text_delta")$.onTextDelta?.(z?.delta||"");else if(Y==="side_prompt_done")Z=z;else if(Y==="side_prompt_error")N=z}),N){let Y=Error(N?.error||"Side prompt failed");throw Y.payload=N,Y}return Z}async function k4(_,$){let j=await fetch("/agent/whitelist",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({pattern:_,description:$})});if(!j.ok){let Z=await j.json().catch(()=>({error:"Failed to add to whitelist"}));throw Error(Z.error||`HTTP ${j.status}`)}return j.json()}async function P4(_,$="thought"){let j=`/agent/thought?turn_id=${encodeURIComponent(_)}&panel=${encodeURIComponent($)}`;return i_(j)}async function M4(_,$,j){return i_("/agent/thought/visibility",{method:"POST",body:JSON.stringify({turn_id:_,panel:$,expanded:Boolean(j)})})}function J$(_){return`/media/${_}`}function I4(_){return`/media/${_}/thumbnail`}async function v2(_){let $=await fetch(`/media/${_}/info`);if(!$.ok)throw Error("Failed to get media info");return $.json()}async function T4(_){let $=await fetch(`/media/${_}`);if(!$.ok)throw Error("Failed to load media text");return $.text()}async function Q7(_){let $=await fetch(`/media/${_}`);if(!$.ok)throw Error("Failed to load media blob");return $.blob()}async function j1(_="",$=2,j=!1){let Z=`/workspace/tree?path=${encodeURIComponent(_)}&depth=${$}&show_hidden=${j?"1":"0"}`;return i_(Z)}async function q7(_=""){let $=`/workspace/branch?path=${encodeURIComponent(_||"")}`;return i_($)}async function x4(_,$=20000,j=null){let Z=j?`&mode=${encodeURIComponent(j)}`:"",N=`/workspace/file?path=${encodeURIComponent(_)}&max=${$}${Z}`;return i_(N)}async function X7(_,$){return i_("/workspace/file",{method:"PUT",body:JSON.stringify({path:_,content:$})})}async function w4(_){return i_("/workspace/attach",{method:"POST",body:JSON.stringify({path:_})})}async function P1(_,$="",j={}){let Z=new FormData;Z.append("file",_);let N=new URLSearchParams;if($)N.set("path",$);if(j.overwrite)N.set("overwrite","1");let Y=N.toString(),z=Y?`/workspace/upload?${Y}`:"/workspace/upload",G=await fetch(""+z,{method:"POST",body:Z});if(!G.ok){let W=await G.json().catch(()=>({error:"Upload failed"})),Q=Error(W.error||`HTTP ${G.status}`);throw Q.status=G.status,Q.code=W.code,Q}return G.json()}async function f4(_,$,j=""){let Z=await fetch("/workspace/file",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({path:_,name:$,content:j})});if(!Z.ok){let N=await Z.json().catch(()=>({error:"Create failed"})),Y=Error(N.error||`HTTP ${Z.status}`);throw Y.status=Z.status,Y.code=N.code,Y}return Z.json()}async function C4(_,$){let j=await fetch("/workspace/rename",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({path:_,name:$})});if(!j.ok){let Z=await j.json().catch(()=>({error:"Rename failed"})),N=Error(Z.error||`HTTP ${j.status}`);throw N.status=j.status,N.code=Z.code,N}return j.json()}async function b4(_,$){let j=await fetch("/workspace/move",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({path:_,target:$})});if(!j.ok){let Z=await j.json().catch(()=>({error:"Move failed"})),N=Error(Z.error||`HTTP ${j.status}`);throw N.status=j.status,N.code=Z.code,N}return j.json()}async function S4(_){let $=`/workspace/file?path=${encodeURIComponent(_||"")}`;return i_($,{method:"DELETE"})}async function Z1(_,$=!1){return i_("/workspace/visibility",{method:"POST",body:JSON.stringify({visible:Boolean(_),show_hidden:Boolean($)})})}function M1(_){return`/workspace/raw?path=${encodeURIComponent(_)}`}function I1(_,$=!1){return`/workspace/download?${`path=${encodeURIComponent(_||"")}&show_hidden=${$?"1":"0"}`}`}class T1{constructor(_,$,j={}){this.onEvent=_,this.onStatusChange=$,this.chatJid=typeof j?.chatJid==="string"&&j.chatJid.trim()?j.chatJid.trim():null,this.eventSource=null,this.reconnectTimeout=null,this.reconnectDelay=1000,this.status="disconnected",this.reconnectAttempts=0,this.cooldownUntil=0,this.connecting=!1,this.lastActivityAt=0,this.staleCheckTimer=null,this.staleThresholdMs=70000}markActivity(){this.lastActivityAt=Date.now()}clearStaleMonitor(){if(this.staleCheckTimer)clearInterval(this.staleCheckTimer),this.staleCheckTimer=null}startStaleMonitor(){this.clearStaleMonitor(),this.staleCheckTimer=setInterval(()=>{if(this.status!=="connected")return;if(!this.lastActivityAt)return;if(Date.now()-this.lastActivityAt<=this.staleThresholdMs)return;console.warn("SSE connection went stale; forcing reconnect"),this.forceReconnect()},15000)}forceReconnect(){if(this.connecting=!1,this.eventSource)this.eventSource.close(),this.eventSource=null;this.clearStaleMonitor(),this.status="disconnected",this.onStatusChange("disconnected"),this.reconnectAttempts+=1,this.scheduleReconnect()}connect(){if(this.connecting)return;if(this.eventSource&&this.status==="connected")return;if(this.connecting=!0,this.eventSource)this.eventSource.close();this.clearStaleMonitor();let _=this.chatJid?`?chat_jid=${encodeURIComponent(this.chatJid)}`:"";this.eventSource=new EventSource("/sse/stream"+_);let $=(j)=>{this.eventSource.addEventListener(j,(Z)=>{this.markActivity(),this.onEvent(j,JSON.parse(Z.data))})};this.eventSource.onopen=()=>{this.connecting=!1,this.reconnectDelay=1000,this.reconnectAttempts=0,this.cooldownUntil=0,this.status="connected",this.markActivity(),this.startStaleMonitor(),this.onStatusChange("connected")},this.eventSource.onerror=()=>{this.connecting=!1,this.clearStaleMonitor(),this.status="disconnected",this.onStatusChange("disconnected"),this.reconnectAttempts+=1,this.scheduleReconnect()},this.eventSource.addEventListener("connected",()=>{this.markActivity(),console.log("SSE connected"),this.onEvent("connected",{})}),this.eventSource.addEventListener("heartbeat",()=>{this.markActivity()}),$("new_post"),$("new_reply"),$("agent_response"),$("interaction_updated"),$("interaction_deleted"),$("agent_status"),$("agent_steer_queued"),$("agent_followup_queued"),$("agent_followup_consumed"),$("agent_followup_removed"),$("workspace_update"),$("agent_draft"),$("agent_draft_delta"),$("agent_thought"),$("agent_thought_delta"),$("model_changed"),$("ui_theme"),["extension_ui_request","extension_ui_timeout","extension_ui_notify","extension_ui_status","extension_ui_working","extension_ui_widget","extension_ui_title","extension_ui_editor_text","extension_ui_error"].forEach($)}scheduleReconnect(){if(this.reconnectTimeout)clearTimeout(this.reconnectTimeout);let _=10,$=60000,j=Date.now();if(this.reconnectAttempts>=_)this.cooldownUntil=Math.max(this.cooldownUntil,j+$),this.reconnectAttempts=0;let Z=Math.max(this.cooldownUntil-j,0),N=Math.max(this.reconnectDelay,Z);this.reconnectTimeout=setTimeout(()=>{console.log("Reconnecting SSE..."),this.connect()},N),this.reconnectDelay=Math.min(this.reconnectDelay*2,30000)}reconnectIfNeeded(){let _=Date.now();if(this.status==="connected"){if(this.lastActivityAt&&_-this.lastActivityAt>this.staleThresholdMs)this.forceReconnect();return}if(this.cooldownUntil&&_<this.cooldownUntil)return;if(this.reconnectTimeout)clearTimeout(this.reconnectTimeout),this.reconnectTimeout=null;this.connect()}disconnect(){if(this.connecting=!1,this.clearStaleMonitor(),this.eventSource)this.eventSource.close(),this.eventSource=null;if(this.reconnectTimeout)clearTimeout(this.reconnectTimeout),this.reconnectTimeout=null}}function x1(_){return String(_||"").toLowerCase().replace(/^@/,"").replace(/\s+/g," ").trim()}function W7(_,$){let j=x1(_),Z=x1($);if(!Z)return!1;return j.startsWith(Z)||j.includes(Z)}function R4(_){if(!_)return!1;if(_.isComposing)return!1;if(_.ctrlKey||_.metaKey||_.altKey)return!1;return typeof _.key==="string"&&_.key.length===1&&/\S/.test(_.key)}function v4(_,$,j=Date.now(),Z=700){let N=_&&typeof _==="object"?_:{value:"",updatedAt:0},Y=String($||"").trim().toLowerCase();if(!Y)return{value:"",updatedAt:j};return{value:!N.value||!Number.isFinite(N.updatedAt)||j-N.updatedAt>Z?Y:`${N.value}${Y}`,updatedAt:j}}function L7(_,$){let j=Math.max(0,Number(_)||0);if(j<=0)return[];let N=((Number.isInteger($)?$:0)%j+j)%j,Y=[];for(let z=0;z<j;z+=1)Y.push((N+z)%j);return Y}function B7(_,$,j=0,Z=(N)=>N){let N=x1($);if(!N)return-1;let Y=Array.isArray(_)?_:[],z=L7(Y.length,j),G=Y.map((W)=>x1(Z(W)));for(let W of z)if(G[W].startsWith(N))return W;for(let W of z)if(G[W].includes(N))return W;return-1}function u4(_,$,j=-1,Z=(N)=>N){let N=Array.isArray(_)?_:[];if(j>=0&&j<N.length){let Y=Z(N[j]);if(W7(Y,$))return j}return B7(N,$,0,Z)}function z$(_){if(typeof window>"u"||!window.localStorage)return null;try{return window.localStorage.getItem(_)}catch{return null}}function e_(_,$){if(typeof window>"u"||!window.localStorage)return;try{window.localStorage.setItem(_,$)}catch{}}function u2(_,$=!1){let j=z$(_);if(j===null)return $;return j==="true"}function m2(_,$=null){let j=z$(_);if(j===null)return $;let Z=parseInt(j,10);return Number.isFinite(Z)?Z:$}function w1(_){return String(_||"").trim().toLowerCase()}function m4(_){let $=String(_||"").match(/^@([a-zA-Z0-9_-]*)$/);if(!$)return null;return w1($[1]||"")}function k8(_){let $=new Set,j=[];for(let Z of Array.isArray(_)?_:[]){let N=w1(Z?.agent_name);if(!N||$.has(N))continue;$.add(N),j.push(Z)}return j}function P8(_,$,j={}){let Z=m4($);if(Z==null)return[];let N=typeof j?.currentChatJid==="string"?j.currentChatJid:null;return k8(_).filter((Y)=>{if(N&&Y?.chat_jid===N)return!1;return w1(Y?.agent_name).startsWith(Z)})}function g4(_){let $=w1(_);return $?`@${$} `:""}function M8(_,$={}){let j=typeof $?.currentChatJid==="string"?$.currentChatJid:null,Z=Number.isFinite($?.limit)?Math.max(0,$.limit):4;return k8(_).filter((N)=>!(j&&N?.chat_jid===j)).slice(0,Z)}function I8({footerWidth:_=0,visibleAgentCount:$=0,hasContextIndicator:j=!1}={}){let Z=Number(_||0),N=Math.max(0,Math.min(Number($||0),4));if(!Number.isFinite(Z)||Z<=0)return!1;if(N<=0)return!1;let Y=460+N*68+(j?40:0);return Z>=Y}function b$({prefix:_="file",label:$,title:j,onRemove:Z,onClick:N,removeTitle:Y="Remove",icon:z="file"}){let G=`${_}-file-pill`,W=`${_}-file-name`,Q=`${_}-file-remove`,L=z==="message"?B`<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
  ...
  `}var U7=[{name:"/model",description:"Select model or list available models"},{name:"/cycle-model",description:"Cycle to the next available model"},{name:"/thinking",description:"Show or set thinking level"},{name:"/cycle-thinking",description:"Cycle thinking level"},{name:"/theme",description:"Set UI theme (no name to show available themes)"},{name:"/tint",description:"Tint default light/dark UI (usage: /tint #hex or /tint off)"},{name:"/btw",description:"Open a side conversation panel without interrupting the main chat"},{name:"/state",description:"Show current session state"},{name:"/stats",description:"Show session token and cost stats"},{name:"/context",description:"Show context window usage"},{name:"/last",description:"Show last assistant response"},{name:"/compact",description:"Manually compact the session"},{name:"/auto-compact",description:"Toggle auto-compaction"},{name:"/auto-retry",description:"Toggle auto-retry"},{name:"/abort",description:"Abort the current response"},{name:"/abort-retry",description:"Abort retry backoff"},{name:"/abort-bash",description:"Abort running bash command"},{name:"/shell",description:"Run a shell command and return output"},{name:"/bash",description:"Run a shell command and add output to context"},{name:"/queue",description:"Queue a follow-up message (one-at-a-time)"},{name:"/queue-all",description:"Queue a follow-up message (batch all)"},{name:"/steer",description:"Steer the current response"},{name:"/steering-mode",description:"Set steering mode (all|one)"},{name:"/followup-mode",description:"Set follow-up mode (all|one)"},{name:"/session-name",description:"Set or show the session name"},{name:"/new-session",description:"Start a new session"},{name:"/switch-session",description:"Switch to a session file"},{name:"/fork",description:"Fork from a previous message"},{name:"/forks",description:"List forkable messages"},{name:"/tree",description:"List the session tree"},{name:"/label",description:"Set or clear a label on a tree entry"},{name:"/labels",description:"List labeled entries"},{name:"/agent-name",description:"Set or show the agent display name"},{name:"/agent-avatar",description:"Set or show the agent avatar URL"},{name:"/user-name",description:"Set or show your display name"},{name:"/user-avatar",description:"Set or show your avatar URL"},{name:"/user-github",description:"Set name/avatar from GitHub profile"},{name:"/export-html",description:"Export session to HTML"},{name:"/passkey",description:"Manage passkeys (enrol/list/delete)"},{name:"/totp",description:"Show a TOTP enrolment QR code"},{name:"/qr",description:"Generate a QR code for text or URL"},{name:"/search",description:"Search notes and skills in the workspace"},{name:"/restart",description:"Restart the agent and stop subprocesses"},{name:"/commands",description:"List available commands"}];function O7({usage:_,onCompact:$}){let j=Math.min(100,Math.max(0,_.percent||0)),Z=_.tokens,N=_.contextWindow,Y="Compact context",G=`${Z!=null?`Context: ${T8(Z)} / ${T8(N)} tokens (${j.toFixed(0)}%)`:`Context: ${j.toFixed(0)}%`} — ${"Compact context"}`,W=9,Q=2*Math.PI*9,L=j/100*Q,X=j>90?"var(--context-red, #ef4444)":j>75?"var(--context-amber, #f59e0b)":"var(--context-green, #22c55e)";return B`
    ...
    `}function T8(_){if(_==null)return"?";if(_>=1e6)return(_/1e6).toFixed(1)+"M";if(_>=1000)return(_/1000).toFixed(0)+"K";return String(_)}function x8({onPost:_,onFocus:$,searchMode:j,searchScope:Z="current",onSearch:N,onSearchScopeChange:Y,onEnterSearch:z,onExitSearch:G,fileRefs:W=[],onRemoveFileRef:Q,onClearFileRefs:L,messageRefs:X=[],onRemoveMessageRef:F,onClearMessageRefs:P,activeModel:C=null,modelUsage:g=null,thinkingLevel:u=null,supportsThinking:E=!1,contextUsage:x=null,onContextCompact:m,notificationsEnabled:A=!1,notificationPermission:S="default",onToggleNotifications:a,onModelChange:Z_,onModelStateChange:N_,activeEditorPath:e=null,onAttachEditorFile:r,onOpenFilePill:z_,followupQueueItems:f0=[],onInjectQueuedFollowup:y_,onRemoveQueuedFollowup:$_,onSubmitIntercept:Y_,onMessageResponse:V_,onPopOutChat:X_,isAgentActive:U_=!1,activeChatAgents:W_=[],currentChatJid:k_="web:default",connectionStatus:G_="connected",onSetFileRefs:d_,onSetMessageRefs:l_,onSubmitError:V0,onSwitchChat:T_,onRenameSession:$0,isRenameSessionInProgress:g_=!1,onCreateSession:C0,onDeleteSession:d,onRestoreSession:x_}){let[Q_,D_]=v(""),[R_,G0]=v(""),[n_,s_]=v([]),[p_,o_]=v(!1),[w_,b_]=v([]),[c_,x0]=v(0),[J0,r_]=v(!1),[Q0,X0]=v([]),[S_,E0]=v(0),[w0,P_]=v(!1),[Y0,W0]=v(!1),[E_,L0]=v(!1),[f_,C_]=v(!1),[v_,e0]=v([]),[w,s]=v(0),[F_,M_]=v(0),[j0,i0]=v(!1),[y$,V$]=v(0),[A0,_$]=v(null),A_=k(null),N2=k(null),b0=k(null),G$=k(null),k$=k(null),h$=k(null),L2=k(null),s0=k(null),$$=k({value:"",updatedAt:0}),B0=k(0),Z0=k(!1),i$=200,a_=(q)=>{let J=new Set,f=[];for(let h of q||[]){if(typeof h!=="string")continue;let K_=h.trim();if(!K_||J.has(K_))continue;J.add(K_),f.push(K_)}return f},v0=()=>{let q=z$("piclaw_compose_history");if(!q)return[];try{let J=JSON.parse(q);if(!Array.isArray(J))return[];return a_(J)}catch{return[]}},S0=(q)=>{e_("piclaw_compose_history",JSON.stringify(q))},Q$=k(v0()),q$=k(-1),P$=k(""),x2=Q_.trim()||n_.length>0||W.length>0||X.length>0,M$=typeof window<"u"&&typeof navigator<"u"&&Boolean(navigator.geolocation)&&Boolean(window.isSecureContext),B2=typeof window<"u"&&typeof Notification<"u",w2=typeof window<"u"?Boolean(window.isSecureContext):!1,l$=B2&&w2&&S!=="denied",j$=S==="granted"&&A,X$=j$?"Disable notifications":"Enable notifications",W$=n_.length>0||W.length>0||X.length>0,y0=G_==="disconnected"?"Reconnecting":String(G_||"Connecting").replace(/[-_]+/g," ").replace(/^./,(q)=>q.toUpperCase()),o0=G_==="disconnected"?"Reconnecting":`Connection: ${y0}`,n$=(Array.isArray(W_)?W_:[]).filter((q)=>!q?.archived_at),S$=M8(n$,{currentChatJid:k_,limit:4}),U0=!j&&I8({footerWidth:y$,visibleAgentCount:S$.length,hasContextIndicator:Boolean(x&&x.percent!=null)}),O0=(()=>{for(let q of Array.isArray(W_)?W_:[]){let J=typeof q?.chat_jid==="string"?q.chat_jid.trim():"";if(J&&J===k_)return q}return null})(),l0=Boolean(O0&&O0.chat_jid===(O0.root_chat_jid||O0.chat_jid)),d$=I_(()=>{let q=new Set,J=[];for(let f of Array.isArray(W_)?W_:[]){let h=typeof f?.chat_jid==="string"?f.chat_jid.trim():"";if(!h||h===k_||q.has(h))continue;if(!(typeof f?.agent_name==="string"?f.agent_name.trim():""))continue;q.add(h),J.push(f)}return J},[W_,k_]),R$=d$.length>0,v$=R$&&typeof T_==="function",u0=R$&&typeof x_==="function",m0=Boolean(g_||Z0.current),t_=!j&&typeof $0==="function"&&!m0,n0=!j&&typeof C0==="function",L$=!j&&typeof d==="function"&&!l0,B$=!j&&(v$||u0||t_||n0||L$),r0=C||"",k0=E&&u?` (${u})`:"",I$=k0.trim()?`${u}`:"",s$=typeof g?.hint_short==="string"?g.hint_short.trim():"",Y2=[I$||null,s$||null].filter(Boolean).join(" • "),T$=[r0?`Current model: ${r0}${k0}`:null,g?.plan?`Plan: ${g.plan}`:null,s$||null,g?.primary?.reset_description||null,g?.secondary?.reset_description||null].filter(Boolean),u$=Y0?"Switching model…":T$.join(" • ")||`Current model: ${r0}${k0} (tap to open model picker)`,o$=(q)=>{if(!q||typeof q!=="object")return;let J=q.model??q.current;if(typeof N_==="function")N_({model:J??null,thinking_level:q.thinking_level??null,supports_thinking:q.supports_thinking,provider_usage:q.provider_usage??null});if(J&&typeof Z_==="function")Z_(J)},P0=(q)=>{let J=q||A_.current;if(!J)return;J.style.height="auto",J.style.height=`${J.scrollHeight}px`,J.style.overflowY="hidden"},x$=(q)=>{if(!q)return{content:q,fileRefs:[]};let f=q.replace(/\r\n/g,`
`).replace(/\r/g,`
`).split(`
                    ...
                    ${W$&&B`
                        <div class="compose-file-refs">
                            ...
                            ${X.map((q)=>{return B`
                                    ...
                                    />
                                `})}
                            ${W.map((q)=>{let J=q.split("/").pop()||q;return B`
                                    <${b$}
                                        prefix="compose"
                                        label=${J}
                                        title=${q}
...
${j}
\`\`\`
`;return`\`${j}\``})}var m7={span:new Set(["title","class","lang","dir"])};function g7(_,$){let j=m7[_];if(!j||!$)return"";let Z=[],N=/([a-zA-Z_:][\w:.-]*)(?:\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s"'`=<>]+)))?/g,Y;while(Y=N.exec($)){let z=(Y[1]||"").toLowerCase();if(!z||z.startsWith("on")||!j.has(z))continue;let G=Y[2]??Y[3]??Y[4]??"";Z.push(` ${z}="${n4(G)}"`)}return Z.join("")}function s8(_){if(!_)return _;return _.replace(/&lt;([\s\S]*?)&gt;/g,($,j)=>{let Z=j.trim(),N=Z.startsWith("/"),Y=N?Z.slice(1).trim():Z,G=Y.endsWith("/")?Y.slice(0,-1).trim():Y,[W=""]=G.split(/\s+/,1),Q=W.toLowerCase();if(!Q||!T7.has(Q))return $;if(Q==="br")return N?"":"<br>";if(N)return`</${Q}>`;let L=G.slice(W.length).trim(),X=g7(Q,L);return`<${Q}${X}>`})}function o8(_){if(!_)return _;let $=(j)=>j.replace(/&amp;lt;/g,"&lt;").replace(/&amp;gt;/g,"&gt;").replace(/&amp;quot;/g,"&quot;").replace(/&amp;#39;/g,"&#39;").replace(/&amp;amp;/g,"&amp;");return _.replace(/<pre><code>([\s\S]*?)<\/code><\/pre>/g,(j,Z)=>`<pre><code>${$(Z)}</code></pre>`).replace(/<code>([\s\S]*?)<\/code>/g,(j,Z)=>`<code>${$(Z)}</code>`)}function r8(_){if(!_)return _;let $=new DOMParser().parseFromString(_,"text/html"),j=$.createTreeWalker($.body,NodeFilter.SHOW_TEXT),Z=(Y)=>Y.replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&quot;/g,'"').replace(/&#39;/g,"'").replace(/&amp;/g,"&"),N;while(N=j.nextNode()){if(!N.nodeValue)continue;let Y=Z(N.nodeValue);if(Y!==N.nodeValue)N.nodeValue=Y}return $.body.innerHTML}function p7(_){if(!window.katex)return _;let $=(z)=>n8(z).replace(/&gt;/g,">").replace(/&lt;/g,"<").replace(/&amp;/g,"&").replace(/<br\s*\/?\s*>/gi,`
`),j=(z)=>{let G=[],W=z.replace(/<pre\b[^>]*>\s*<code\b[^>]*>[\s\S]*?<\/code>\s*<\/pre>/gi,(Q)=>{let L=G.length;return G.push(Q),`@@CODE_BLOCK_${L}@@`});return W=W.replace(/<code\b[^>]*>[\s\S]*?<\/code>/gi,(Q)=>{let L=G.length;return G.push(Q),`@@CODE_INLINE_${L}@@`}),{html:W,blocks:G}},Z=(z,G)=>{if(!G.length)return z;return z.replace(/@@CODE_(?:BLOCK|INLINE)_(\d+)@@/g,(W,Q)=>{let L=Number(Q);return G[L]??""})},N=j(_),Y=N.html;return Y=Y.replace(/(^|\n|<br\s*\/?\s*>|<p>|<\/p>)\s*\$\$([\s\S]+?)\$\$\s*(?=\n|<br\s*\/?\s*>|<\/p>|$)/gi,(z,G,W)=>{try{let Q=katex.renderToString($(W.trim()),{displayMode:!0,throwOnError:!1});return`${G}${Q}`}catch(Q){return`<span class="math-error" title="${n4(Q.message)}">${z}</span>`}}),Y=Y.replace(/(^|[^\\$])\$(?!\s)([^\n$]+?)\$/g,(z,G,W)=>{if(/\s$/.test(W))return z;try{let Q=katex.renderToString($(W),{displayMode:!1,throwOnError:!1});return`${G}${Q}`}catch(Q){return`${G}<span class="math-error" title="${n4(Q.message)}">$${W}$</span>`}}),Z(Y,N.blocks)}function c7(_){if(!_)return _;let $=new DOMParser().parseFromString(_,"text/html"),j=$.createTreeWalker($.body,NodeFilter.SHOW_TEXT),Z=[],N;while(N=j.nextNode())Z.push(N);for(let Y of Z){let z=Y.nodeValue;if(!z)continue;if(S1.lastIndex=0,!S1.test(z))continue;S1.lastIndex=0;let G=Y.parentElement;if(G&&(G.closest("a")||G.closest("code")||G.closest("pre")))continue;let W=z.split(S1);if(W.length<=1)continue;let Q=$.createDocumentFragment();W.forEach((L,X)=>{if(X%2===1){let F=$.createElement("a");F.setAttribute("href","#"),F.className="hashtag",F.setAttribute("data-hashtag",L),F.textContent=`#${L}`,Q.appendChild(F)}else Q.appendChild($.createTextNode(L))}),Y.parentNode?.replaceChild(Q,Y)}return $.body.innerHTML}function h7(_){if(!_)return _;let j=_.replace(/\r\n/g,`
`).replace(/\r/g,`
`).split(`
...
`)}function K$(_,$,j={}){if(!_)return"";let Z=h7(_),{text:N,blocks:Y}=b7(Z),z=R1(N,2),W=d8(z).replace(/</g,"&lt;").replace(/>/g,"&gt;"),Q=s8(W),L=window.marked?marked.parse(Q,{headerIds:!1,mangle:!1}):Q.replace(/\n/g,"<br>");return L=o8(L),L=r8(L),L=p7(L),L=c7(L),L=u7(L,Y),L=l8(L,j),L}function v1(_){if(!_)return"";let $=_.replace(/\r\n/g,`
`).replace(/\r/g,`
`),j=R1($,2),N=d8(j).replace(/</g,"&lt;").replace(/>/g,"&gt;"),Y=s8(N),z=window.marked?marked.parse(Y):Y.replace(/\n/g,"<br>");return z=o8(z),z=r8(z),z=l8(z),z}function i7(_,$=6){return _.replace(/<polyline\b([^>]*)\bpoints="([^"]+)"([^>]*)\/?\s*>/g,(j,Z,N,Y)=>{let z=N.trim().split(/\s+/).map((W)=>{let[Q,L]=W.split(",").map(Number);return{x:Q,y:L}});if(z.length<3)return`<polyline${Z}points="${N}"${Y}/>`;let G=[`M ${z[0].x},${z[0].y}`];for(let W=1;W<z.length-1;W++){let Q=z[W-1],L=z[W],X=z[W+1],F=L.x-Q.x,P=L.y-Q.y,C=X.x-L.x,g=X.y-L.y,u=Math.sqrt(F*F+P*P),E=Math.sqrt(C*C+g*g),x=Math.min($,u/2,E/2);if(x<0.5){G.push(`L ${L.x},${L.y}`);continue}let m=L.x-F/u*x,A=L.y-P/u*x,S=L.x+C/E*x,a=L.y+g/E*x,N_=F*g-P*C>0?1:0;G.push(`L ${m},${A}`),G.push(`A ${x},${x} 0 0 ${N_} ${S},${a}`)}return G.push(`L ${z[z.length-1].x},${z[z.length-1].y}`),`<path${Z}d="${G.join(" ")}"${Y}/>`})}async function j2(_){if(!window.beautifulMermaid)return;let{renderMermaid:$,THEMES:j}=window.beautifulMermaid,N=i8()==="dark"?j["tokyo-night"]:j["github-light"],Y=_.querySelectorAll(".mermaid-container[data-mermaid]");for(let z of Y)try{let G=z.dataset.mermaid,W=v7(G||""),Q=R1(W,2),L=await $(Q,{...N,transparent:!0});L=i7(L),z.innerHTML=L,z.removeAttribute("data-mermaid")}catch(G){console.error("Mermaid render error:",G);let W=document.createElement("pre");W.className="mermaid-error",W.textContent=`Diagram error: ${G.message}`,z.innerHTML="",z.appendChild(W),z.removeAttribute("data-mermaid")}}function a8(_){let $=String(_||"").trim();if(!$.startsWith("/btw"))return null;let j=$.slice(4).trim();if(!j)return{type:"help"};if(j==="clear"||j==="close")return{type:"clear"};return{type:"ask",question:j}}function t8(_){return String(_||"").trim()||"web:default"}function e8(_){if(!_)return!1;let $=String(_.answer||"").trim();return _.status!=="running"&&Boolean($)}function _6(_){if(!_)return!1;return _.status!=="running"}function $6(_){let $=String(_?.question||"").trim(),j=String(_?.answer||"").trim();if(!$&&!j)return"";return["BTW side conversation",$?`Question: ${$}`:null,j?`Answer:
${j}`:null].filter(Boolean).join(`

`)}function j6({session:_,onClose:$,onInject:j,onRetry:Z}){let N=k(null),Y=k(null),z=_?.thinking?v1(_.thinking):"",G=_?.answer?K$(_.answer,null,{sanitize:!1}):"";if(b(()=>{if(N.current&&z)j2(N.current).catch(()=>{})},[z]),b(()=>{if(Y.current&&G)j2(Y.current).catch(()=>{})},[G]),!_)return null;let W=_.status==="running",Q=Boolean(String(_.answer||"").trim()),L=Boolean(String(_.thinking||"").trim()),X=e8(_),F=_6(_),P=!W&&Q,C=W?"Thinking…":_.status==="error"?"Error":"Done";return B`
...
</script>`}function W6(_){let $=_?.artifact||{},j=$.kind||_?.kind||null,Z=typeof $.html==="string"?$.html:typeof _?.html==="string"?_.html:"",N=typeof $.svg==="string"?$.svg:typeof _?.svg==="string"?_.svg:"",Y=typeof _?.title==="string"&&_.title.trim()?_.title.trim():"Generated widget",z=j==="svg"?N:Z;if(!z)return"";let G=V6(_),W=["default-src 'none'","img-src data: blob: https: http:","style-src 'unsafe-inline'","font-src data: https: http:","media-src data: blob: https: http:","connect-src 'none'","frame-src 'none'",G?"script-src 'unsafe-inline'":"script-src 'none'","object-src 'none'","base-uri 'none'","form-action 'none'"].join("; "),Q=j==="svg"?`<div class="widget-svg-shell">${z}</div>`:z,L=G?d7(_):"";return`<!doctype html>
<html>
<head>
<meta charset="utf-8" />
...
<body>${Q}</body>
</html>`}function L6({widget:_,onClose:$,onWidgetEvent:j}){let Z=k(null),N=k(!1),Y=I_(()=>W6(_),[_?.artifact?.kind,_?.artifact?.html,_?.artifact?.svg,_?.widgetId,_?.toolCallId,_?.turnId,_?.title]);if(b(()=>{if(!_)return;let E=(x)=>{if(x.key==="Escape")$?.()};return document.addEventListener("keydown",E),()=>document.removeEventListener("keydown",E)},[_,$]),b(()=>{N.current=!1},[Y]),b(()=>{if(!_)return;let E=Z.current;if(!E)return;let x=(Z_)=>{let N_=g1(_),e=Z_==="widget.init"?u1(_):m1(_);try{E.name=N_}catch{}try{E.contentWindow?.postMessage({__piclawGeneratedWidgetHost:!0,type:Z_,widgetId:_?.widgetId||null,toolCallId:_?.toolCallId||null,turnId:_?.turnId||null,payload:e},"*")}catch{}},m=()=>{x("widget.init"),x("widget.update")},A=()=>{N.current=!0,m()};E.addEventListener("load",A);let a=[0,40,120,300,800].map((Z_)=>setTimeout(m,Z_));return()=>{E.removeEventListener("load",A),a.forEach((Z_)=>clearTimeout(Z_))}},[Y,_?.widgetId,_?.toolCallId,_?.turnId]),b(()=>{if(!_)return;let E=Z.current;if(!E?.contentWindow)return;let x=g1(_),m=m1(_);try{E.name=x}catch{}try{E.contentWindow.postMessage({__piclawGeneratedWidgetHost:!0,type:"widget.update",widgetId:_?.widgetId||null,toolCallId:_?.toolCallId||null,turnId:_?.turnId||null,payload:m},"*")}catch{}return},[_?.widgetId,_?.toolCallId,_?.turnId,_?.status,_?.subtitle,_?.description,_?.error,_?.width,_?.height,_?.runtimeState]),b(()=>{if(!_)return;let E=(x)=>{let m=x?.data;if(!m||m.__piclawGeneratedWidget!==!0)return;let A=Z.current,S=c0(_),a=c0({widgetId:m.widgetId,toolCallId:m.toolCallId});if(a&&S&&a!==S)return;if(!a&&A?.contentWindow&&x.source!==A.contentWindow)return;j?.(m,_)};return window.addEventListener("message",E),()=>window.removeEventListener("message",E)},[_,j]),!_)return null;let G=(_?.artifact||{}).kind||_?.kind||"html",W=typeof _?.title==="string"&&_.title.trim()?_.title.trim():"Generated widget",Q=typeof _?.subtitle==="string"&&_.subtitle.trim()?_.subtitle.trim():"",L=_?.source==="live"?"live":"timeline",X=typeof _?.status==="string"&&_.status.trim()?_.status.trim():"final",F=L==="live"?`Live widget • ${X.toUpperCase()}`:_?.originPostId?`Message #${_.originPostId}`:"Timeline launch",P=typeof _?.description==="string"&&_.description.trim()?_.description.trim():"",C=!Y,g=X6(_),u=G6(_);return B`
            ...
            >
                <div class="floating-widget-header">
                    <div class="floating-widget-heading">
                        ...
                        <div class="floating-widget-title">${W}</div>
                        ${(Q||P)&&B`
    ...
    `}var B6="PiClaw";function s4(_,$,j=!1){let Z=_||"PiClaw",N=Z.charAt(0).toUpperCase(),Y=["#FF6B6B","#4ECDC4","#45B7D1","#FFA07A","#98D8C8","#F7DC6F","#BB8FCE","#85C1E2","#F8B195","#6C5CE7","#00B894","#FDCB6E","#E17055","#74B9FF","#A29BFE","#FD79A8","#00CEC9","#FFEAA7","#DFE6E9","#FF7675","#55EFC4","#81ECEC","#FAB1A0","#74B9FF","#A29BFE","#FD79A8"],z=N.charCodeAt(0)%Y.length,G=Y[z],W=Z.trim().toLowerCase(),Q=typeof $==="string"?$.trim():"",L=Q?Q:null,X=j||W==="PiClaw".toLowerCase()||W==="pi";return{letter:N,color:G,image:L||(X?"/static/icon-192.png":null)}}function U6(_,$){if(!_)return"PiClaw";let j=$[_]?.name||_;return j?j.charAt(0).toUpperCase()+j.slice(1):"PiClaw"}function O6(_,$){if(!_)return null;let j=$[_]||{};return j.avatar_url||j.avatarUrl||j.avatar||null}function F6(_){if(!_)return null;if(typeof document<"u"){let Y=document.documentElement,z=Y?.dataset?.colorTheme||"",G=Y?.dataset?.tint||"",W=getComputedStyle(Y).getPropertyValue("--accent-color")?.trim();if(W&&(G||z&&z!=="default"))return W}let $=["#4ECDC4","#FF6B6B","#45B7D1","#BB8FCE","#FDCB6E","#00B894","#74B9FF","#FD79A8","#81ECEC","#FFA07A"],j=String(_),Z=0;for(let Y=0;Y<j.length;Y+=1)Z=(Z*31+j.charCodeAt(Y))%2147483647;let N=Math.abs(Z)%$.length;return $[N]}function s7(_){if(!_||typeof _!=="object")return null;let $=_.started_at??_.startedAt;if(typeof $!=="string"||!$)return null;let j=Date.parse($);return Number.isFinite(j)?j:null}function z1(_){if(!_||typeof _!=="object")return!1;let $=_.intent_key??_.intentKey;return _.type==="intent"&&$==="compaction"}function H6(_){if(!_||typeof _!=="object")return"";let $=_.title;if(typeof $==="string"&&$.trim())return $.trim();let j=_.status;if(typeof j==="string"&&j.trim())return j.trim();return z1(_)?"Compacting context":"Working..."}function o7(_){let $=Math.max(0,Math.floor(_/1000)),j=$%60,Z=Math.floor($/60)%60,N=Math.floor($/3600);if(N>0)return`${N}:${String(Z).padStart(2,"0")}:${String(j).padStart(2,"0")}`;return`${Z}:${String(j).padStart(2,"0")}`}function D6(_,$=Date.now()){let j=s7(_);if(j===null)return null;return o7(Math.max(0,$-j))}function J6({status:_,draft:$,plan:j,thought:Z,pendingRequest:N,intent:Y,turnId:z,steerQueued:G,onPanelToggle:W}){let X=(d)=>{if(!d)return{text:"",totalLines:0,fullText:""};if(typeof d==="string"){let R_=d,G0=R_?R_.replace(/\r\n/g,`
`).split(`
...
`).reduce((o_,w_)=>o_+C(w_),0):0,p_=Math.max(n_-s_,0);return{text:G0,omitted:p_,totalLines:n_,visibleLines:s_}},u=X(j),E=X(Z),x=X($),m=Boolean(u.text)||u.totalLines>0,A=Boolean(E.text)||E.totalLines>0,S=Boolean(x.fullText?.trim()||x.text?.trim());if(!_&&!S&&!m&&!A&&!N&&!Y)return null;let[a,Z_]=v(new Set),[N_,e]=v(()=>Date.now()),r=(d)=>Z_((x_)=>{let Q_=new Set(x_),D_=!Q_.has(d);if(D_)Q_.add(d);else Q_.delete(d);if(typeof W==="function")W(d,D_);return Q_});b(()=>{Z_(new Set)},[z]);let z_=z1(_);b(()=>{if(!z_)return;e(Date.now());let d=setInterval(()=>e(Date.now()),1000);return()=>clearInterval(d)},[z_,_?.started_at,_?.startedAt]);let f0=_?.turn_id||z,y_=F6(f0),$_=G?"turn-dot turn-dot-queued":"turn-dot",Y_=(d)=>d,V_=Boolean(_?.last_activity||_?.lastActivity),X_=(d)=>d==="warning"?"#f59e0b":d==="error"?"var(--danger-color)":d==="success"?"var(--success-color)":y_,U_=Y?.kind||"info",W_=X_(U_),k_=X_(_?.kind||(z_?"warning":"info")),G_="",d_=_?.title,l_=_?.status;if(_?.type==="plan")G_=d_?`Planning: ${d_}`:"Planning...";else if(_?.type==="tool_call")G_=d_?`Running: ${d_}`:"Running tool...";else if(_?.type==="tool_status")G_=d_?`${d_}: ${l_||"Working..."}`:l_||"Working...";else if(_?.type==="error")G_=d_||"Agent error";else G_=d_||l_||"Working...";if(V_)G_="Last activity just now";let V0=({panelTitle:d,text:x_,fullText:Q_,totalLines:D_,maxLines:R_,titleClass:G0,panelKey:n_})=>{let s_=a.has(n_),p_=Q_||x_||"",o_=n_==="thought"||n_==="draft"?P(p_):p_,w_=typeof R_==="number",b_=s_&&w_,c_=w_?g(o_,R_,D_):{text:o_||"",omitted:0,totalLines:Number.isFinite(D_)?D_:0};if(!o_&&!(Number.isFinite(c_.totalLines)&&c_.totalLines>0))return null;let x0=`agent-thinking-body${w_?" agent-thinking-body-collapsible":""}`,J0=w_?`--agent-thinking-collapsed-lines: ${R_};`:"";return B`
        ...
        `},T_=N?.tool_call?.title,$0=T_?`Awaiting approval: ${T_}`:"Awaiting approval",g_=z_?D6(_,N_):null,C0=(d,x_,Q_=null)=>{let D_=H6(d);return B`
            <div
    ...
    `}function E6({request:_,onRespond:$}){if(!_)return null;let{request_id:j,tool_call:Z,options:N,chat_jid:Y}=_,z=Z?.title||"Agent Request",G=Z?.kind||"other",W=Z?.rawInput||{},Q=W.command||W.commands&&W.commands[0]||null,L=W.diff||null,X=W.fileName||W.path||null,F=Z?.description||W.description||W.explanation||null,C=(Array.isArray(Z?.locations)?Z.locations:[]).map((m)=>m?.path).filter((m)=>Boolean(m)),g=Array.from(new Set([X,...C].filter(Boolean)));console.log("AgentRequestModal:",{request_id:j,tool_call:Z,options:N});let u=async(m)=>{try{await k1(j,m,Y||null),$()}catch(A){console.error("Failed to respond to agent request:",A)}},E=async()=>{try{await k4(z,`Auto-approved: ${z}`),await k1(j,"approved",Y||null),$()}catch(m){console.error("Failed to add to whitelist:",m)}},x=N&&N.length>0;return B`
    ...
    `}function A6(_){let $=new Date(_);if(Number.isNaN($.getTime()))return _;let Z=new Date-$,N=Z/1000,Y=86400000;if(Z<Y){if(N<60)return"just now";if(N<3600)return`${Math.floor(N/60)}m`;return`${Math.floor(N/3600)}h`}if(Z<5*Y){let W=$.toLocaleDateString(void 0,{weekday:"short"}),Q=$.toLocaleTimeString(void 0,{hour:"2-digit",minute:"2-digit"});return`${W} ${Q}`}let z=$.toLocaleDateString(void 0,{month:"short",day:"numeric"}),G=$.toLocaleTimeString(void 0,{hour:"2-digit",minute:"2-digit"});return`${z} ${G}`}function K1(_){if(!Number.isFinite(_))return"0";return Math.round(_).toLocaleString()}function E$(_){if(_<1024)return _+" B";if(_<1048576)return(_/1024).toFixed(1)+" KB";return(_/1048576).toFixed(1)+" MB"}function M2(_){let $=new Date(_);if(Number.isNaN($.getTime()))return _;return $.toLocaleString()}var r7=new Set(["application/json","application/xml","text/csv","text/html","text/markdown","text/plain","text/xml"]),a7=new Set(["text/markdown"]),t7=new Set(["application/msword","application/rtf","application/vnd.ms-excel","application/vnd.ms-powerpoint","application/vnd.oasis.opendocument.presentation","application/vnd.oasis.opendocument.spreadsheet","application/vnd.oasis.opendocument.text","application/vnd.openxmlformats-officedocument.presentationml.presentation","application/vnd.openxmlformats-officedocument.spreadsheetml.sheet","application/vnd.openxmlformats-officedocument.wordprocessingml.document"]),e7=new Set(["application/vnd.jgraph.mxfile"]);function V1(_){return typeof _==="string"?_.trim().toLowerCase():""}function _j(_){let $=V1(_);return!!$&&($.endsWith(".drawio")||$.endsWith(".drawio.xml")||$.endsWith(".drawio.svg")||$.endsWith(".drawio.png"))}function $j(_){let $=V1(_);return!!$&&$.endsWith(".pdf")}function jj(_){let $=V1(_);return!!$&&($.endsWith(".docx")||$.endsWith(".doc")||$.endsWith(".odt")||$.endsWith(".rtf")||$.endsWith(".xlsx")||$.endsWith(".xls")||$.endsWith(".ods")||$.endsWith(".pptx")||$.endsWith(".ppt")||$.endsWith(".odp"))}function G1(_,$){let j=V1(_);if(_j($)||e7.has(j))return"drawio";if($j($)||j==="application/pdf")return"pdf";if(jj($)||t7.has(j))return"office";if(!j)return"unsupported";if(j.startsWith("image/"))return"image";if(r7.has(j)||j.startsWith("text/"))return"text";return"unsupported"}function y6(_){let $=V1(_);return a7.has($)}function k6(_){switch(_){case"image":return"Image preview";case"pdf":return"PDF preview";case"office":return"Office viewer";case"drawio":return"Draw.io preview (read-only)";case"text":return"Text preview";default:return"Preview unavailable"}}function Zj(_){let j=String(_||"").trim().match(/^#([0-9a-f]{3}|[0-9a-f]{6})$/i);if(!j)return null;let Z=j[1].length===3?j[1].split("").map((N)=>`${N}${N}`).join(""):j[1];return{r:parseInt(Z.slice(0,2),16),g:parseInt(Z.slice(2,4),16),b:parseInt(Z.slice(4,6),16)}}function Nj(_){let j=String(_||"").trim().match(/^rgba?\((\d+)[,\s]+(\d+)[,\s]+(\d+)/i);if(!j)return null;let Z=Number(j[1]),N=Number(j[2]),Y=Number(j[3]);if(![Z,N,Y].every((z)=>Number.isFinite(z)))return null;return{r:Z,g:N,b:Y}}function P6(_){return Zj(_)||Nj(_)}function p1(_){let $=(Y)=>{let z=Y/255;return z<=0.03928?z/12.92:((z+0.055)/1.055)**2.4},j=$(_.r),Z=$(_.g),N=$(_.b);return 0.2126*j+0.7152*Z+0.0722*N}function Yj(_,$){let j=Math.max(p1(_),p1($)),Z=Math.min(p1(_),p1($));return(j+0.05)/(Z+0.05)}function zj(_,$,j="#ffffff"){let Z=P6(_);if(!Z)return j;let N=j,Y=-1;for(let z of $){let G=P6(z);if(!G)continue;let W=Yj(Z,G);if(W>Y)N=z,Y=W}return N}function o4(){let _=getComputedStyle(document.documentElement),$=(C,g)=>{for(let u of C){let E=_.getPropertyValue(u).trim();if(E)return E}return g},j=$(["--text-primary","--color-text"],"#0f1419"),Z=$(["--text-secondary","--color-text-muted"],"#536471"),N=$(["--bg-primary","--color-bg-primary"],"#ffffff"),Y=$(["--bg-secondary","--color-bg-secondary"],"#f7f9fa"),z=$(["--bg-hover","--bg-tertiary","--color-bg-tertiary"],"#e8ebed"),G=$(["--accent-color","--color-accent"],"#1d9bf0"),W=$(["--success-color","--color-success"],"#00ba7c"),Q=$(["--warning-color","--color-warning","--accent-color"],"#f0b429"),L=$(["--danger-color","--color-error"],"#f4212e"),X=$(["--border-color","--color-border"],"#eff3f4"),F=$(["--font-family"],"system-ui, sans-serif"),P=zj(G,[j,N],j);return{fg:j,fgMuted:Z,bgPrimary:N,bg:Y,bgEmphasis:z,accent:G,good:W,warning:Q,attention:L,border:X,fontFamily:F,buttonTextColor:P}}function M6(){let{fg:_,fgMuted:$,bg:j,bgEmphasis:Z,accent:N,good:Y,warning:z,attention:G,border:W,fontFamily:Q}=o4();return{fontFamily:Q,containerStyles:{default:{backgroundColor:j,foregroundColors:{default:{default:_,subtle:$},accent:{default:N,subtle:N},good:{default:Y,subtle:Y},warning:{default:z,subtle:z},attention:{default:G,subtle:G}}},emphasis:{backgroundColor:Z,foregroundColors:{default:{default:_,subtle:$},accent:{default:N,subtle:N},good:{default:Y,subtle:Y},warning:{default:z,subtle:z},attention:{default:G,subtle:G}}}},actions:{actionsOrientation:"horizontal",actionAlignment:"left",buttonSpacing:8,maxActions:5,showCard:{actionMode:"inline"},spacing:"default"},adaptiveCard:{allowCustomStyle:!1},spacing:{small:4,default:8,medium:12,large:16,extraLarge:24,padding:12},separator:{lineThickness:1,lineColor:W},fontSizes:{small:12,default:14,medium:16,large:18,extraLarge:22},fontWeights:{lighter:300,default:400,bolder:600},imageSizes:{small:40,medium:80,large:120},textBlock:{headingLevel:2}}}var Kj=new Set(["1.0","1.1","1.2","1.3","1.4","1.5","1.6"]),I6=!1,c1=null,T6=!1;function r4(_){_.querySelector(".adaptive-card-notice")?.remove()}function Vj(_,$,j="error"){r4(_);let Z=document.createElement("div");Z.className=`adaptive-card-notice adaptive-card-notice-${j}`,Z.textContent=$,_.appendChild(Z)}function Gj(_,$=(j)=>K$(j,null)){let j=typeof _==="string"?_:String(_??"");if(!j.trim())return{outputHtml:"",didProcess:!1};return{outputHtml:$(j),didProcess:!0}}function Qj(_=($)=>K$($,null)){return($,j)=>{try{let Z=Gj($,_);j.outputHtml=Z.outputHtml,j.didProcess=Z.didProcess}catch(Z){console.error("[adaptive-card] Failed to process markdown:",Z),j.outputHtml=String($??""),j.didProcess=!1}}}function qj(_){if(T6||!_?.AdaptiveCard)return;_.AdaptiveCard.onProcessMarkdown=Qj(),T6=!0}async function Xj(){if(I6)return;if(c1)return c1;return c1=new Promise((_,$)=>{let j=document.createElement("script");j.src="/static/js/vendor/adaptivecards.min.js",j.onload=()=>{I6=!0,_()},j.onerror=()=>$(Error("Failed to load adaptivecards SDK")),document.head.appendChild(j)}),c1}function Wj(){return globalThis.AdaptiveCards}function Lj(_){if(!_||typeof _!=="object")return!1;let $=_;return $.type==="adaptive_card"&&typeof $.card_id==="string"&&typeof $.schema_version==="string"&&typeof $.payload==="object"&&$.payload!==null}function Bj(_){return Kj.has(_)}function t4(_){if(!Array.isArray(_))return[];return _.filter(Lj)}function Uj(_){let $=(typeof _?.getJsonTypeName==="function"?_.getJsonTypeName():"")||_?.constructor?.name||"Unknown",j=(typeof _?.title==="string"?_.title:"")||"",Z=(typeof _?.url==="string"?_.url:"")||void 0,N=_?.data??void 0;return{type:$,title:j,data:N,url:Z,raw:_}}function a4(_){if(_==null)return"";if(typeof _==="string")return _.trim();if(typeof _==="number")return String(_);if(typeof _==="boolean")return _?"yes":"no";if(Array.isArray(_))return _.map(($)=>a4($)).filter(Boolean).join(", ");if(typeof _==="object")return Object.entries(_).map(([j,Z])=>`${j}: ${a4(Z)}`).filter((j)=>!j.endsWith(": ")).join(", ");return String(_).trim()}function Oj(_,$,j){if($==null)return $;if(_==="Input.Toggle"){if(typeof $==="boolean"){if($)return j?.valueOn??"true";return j?.valueOff??"false"}return typeof $==="string"?$:String($)}if(_==="Input.ChoiceSet"){if(Array.isArray($))return $.join(",");return typeof $==="string"?$:String($)}if(Array.isArray($))return $.join(", ");if(typeof $==="object")return a4($);return typeof $==="string"?$:String($)}function Fj(_,$){if(!_||typeof _!=="object")return _;if(!$||typeof $!=="object"||Array.isArray($))return _;let j=$,Z=(N)=>{if(Array.isArray(N))return N.map((G)=>Z(G));if(!N||typeof N!=="object")return N;let z={...N};if(typeof z.id==="string"&&z.id in j&&String(z.type||"").startsWith("Input."))z.value=Oj(z.type,j[z.id],z);for(let[G,W]of Object.entries(z))if(Array.isArray(W)||W&&typeof W==="object")z[G]=Z(W);return z};return Z(_)}function Hj(_){_.classList.add("adaptive-card-readonly");for(let $ of Array.from(_.querySelectorAll("input, textarea, select, button"))){let j=$;try{j.setAttribute("aria-disabled","true")}catch{}try{j.setAttribute("tabindex","-1")}catch{}if("disabled"in j)try{j.disabled=!0}catch{}if("readOnly"in j)try{j.readOnly=!0}catch{}}}function Dj(_){if(typeof _!=="string"||!_.trim())return"";let $=new Date(_);if(Number.isNaN($.getTime()))return"";return new Intl.DateTimeFormat(void 0,{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"}).format($)}function Jj(_){if(_.state==="active")return null;let $=_.state==="completed"?"Submitted":_.state==="cancelled"?"Cancelled":"Failed",j=_.last_submission&&typeof _.last_submission==="object"?_.last_submission:null,Z=j&&typeof j.title==="string"?j.title.trim():"",N=Dj(_.completed_at||j?.submitted_at),Y=[Z||null,N||null].filter(Boolean).join(" · ")||null;return{label:$,detail:Y}}async function x6(_,$,j){if(!Bj($.schema_version))return console.warn(`[adaptive-card] Unsupported schema version ${$.schema_version} for card ${$.card_id}`),!1;try{await Xj()}catch(Z){return console.error("[adaptive-card] Failed to load SDK:",Z),!1}try{let Z=Wj();qj(Z);let N=new Z.AdaptiveCard,Y=o4();N.hostConfig=new Z.HostConfig(M6());let z=$.last_submission&&typeof $.last_submission==="object"?$.last_submission.data:void 0,G=$.state==="active"?$.payload:Fj($.payload,z);N.parse(G),N.onExecuteAction=(L)=>{let X=Uj(L);if(j?.onAction)r4(_),_.classList.add("adaptive-card-busy"),Promise.resolve(j.onAction(X)).catch((F)=>{console.error("[adaptive-card] Action failed:",F);let P=F instanceof Error?F.message:String(F||"Action failed.");Vj(_,P||"Action failed.","error")}).finally(()=>{_.classList.remove("adaptive-card-busy")});else console.log("[adaptive-card] Action executed (not wired yet):",X)};let W=N.render();if(!W)return console.warn(`[adaptive-card] Card ${$.card_id} rendered to null`),!1;_.classList.add("adaptive-card-container"),_.style.setProperty("--adaptive-card-button-text-color",Y.buttonTextColor);let Q=Jj($);if(Q){_.classList.add("adaptive-card-finished");let L=document.createElement("div");L.className=`adaptive-card-status adaptive-card-status-${$.state}`;let X=document.createElement("span");if(X.className="adaptive-card-status-label",X.textContent=Q.label,L.appendChild(X),Q.detail){let F=document.createElement("span");F.className="adaptive-card-status-detail",F.textContent=Q.detail,L.appendChild(F)}_.appendChild(L)}if(r4(_),_.appendChild(W),Q)Hj(W);return!0}catch(Z){return console.error(`[adaptive-card] Failed to render card ${$.card_id}:`,Z),!1}}function Q1(_){if(_==null)return"";if(typeof _==="string")return _.trim();if(typeof _==="number")return String(_);if(typeof _==="boolean")return _?"yes":"no";if(Array.isArray(_))return _.map(($)=>Q1($)).filter(Boolean).join(", ");if(typeof _==="object")return Object.entries(_).filter(([$])=>!$.startsWith("__")).map(([$,j])=>`${$}: ${Q1(j)}`).filter(($)=>!$.endsWith(": ")).join(", ");return String(_).trim()}function w6(_){if(typeof _!=="object"||_==null||Array.isArray(_))return[];return Object.entries(_).filter(([$])=>!$.startsWith("__")).map(([$,j])=>({key:$,value:Q1(j)})).filter(($)=>$.value)}function Ej(_){if(!_||typeof _!=="object")return!1;let $=_;return $.type==="adaptive_card_submission"&&typeof $.card_id==="string"&&typeof $.source_post_id==="number"&&typeof $.submitted_at==="string"}function e4(_){if(!Array.isArray(_))return[];return _.filter(Ej)}function f6(_){let $=String(_.title||_.card_id||"card").trim()||"card",j=_.data;if(j==null)return`Card submission: ${$}`;if(typeof j==="string"||typeof j==="number"||typeof j==="boolean"){let Z=Q1(j);return Z?`Card submission: ${$} — ${Z}`:`Card submission: ${$}`}if(typeof j==="object"){let N=w6(j).map(({key:Y,value:z})=>`${Y}: ${z}`);return N.length>0?`Card submission: ${$} — ${N.join(", ")}`:`Card submission: ${$}`}return`Card submission: ${$}`}function C6(_){let $=String(_.title||_.card_id||"Card submission").trim()||"Card submission",j=w6(_.data),Z=j.length>0?j.slice(0,2).map(({key:Y,value:z})=>`${Y}: ${z}`).join(", "):Q1(_.data)||null,N=j.length;return{title:$,summary:Z,fields:j,fieldCount:N,submittedAt:_.submitted_at}}function Aj(_){let $=_?.metadata?.size;return[{label:"Type",value:_?.content_type||"application/octet-stream"},{label:"Size",value:typeof $==="number"?E$($):null},{label:"Added",value:_?.created_at?M2(_.created_at):null}].filter((Z)=>Z.value)}function yj(_,$,j){let Z=encodeURIComponent($||`attachment-${_}`),N=encodeURIComponent(String(_));if(j==="pdf")return`/pdf-viewer/?media=${N}&name=${Z}#media=${N}&name=${Z}`;if(j==="office"){let Y=J$(_);return`/office-viewer/?url=${encodeURIComponent(Y)}&name=${Z}`}if(j==="drawio")return`/drawio/edit.html?media=${N}&name=${Z}&readonly=1#media=${N}&name=${Z}&readonly=1`;return null}function b6({mediaId:_,info:$,onClose:j}){let Z=$?.filename||`attachment-${_}`,N=I_(()=>G1($?.content_type,Z),[$?.content_type,Z]),Y=k6(N),z=I_(()=>y6($?.content_type),[$?.content_type]),[G,W]=v(N==="text"),[Q,L]=v(""),[X,F]=v(null),P=k(null),C=I_(()=>Aj($),[$]),g=I_(()=>yj(_,Z,N),[_,Z,N]),u=I_(()=>{if(!z||!Q)return"";return K$(Q)},[z,Q]);return b(()=>{let E=(x)=>{if(x.key==="Escape")j()};return document.addEventListener("keydown",E),()=>document.removeEventListener("keydown",E)},[j]),b(()=>{if(!P.current||!u)return;j2(P.current);return},[u]),b(()=>{let E=!1;async function x(){if(N!=="text"){W(!1),F(null);return}W(!0),F(null);try{let m=await T4(_);if(!E)L(m)}catch{if(!E)F("Failed to load text preview.")}finally{if(!E)W(!1)}}return x(),()=>{E=!0}},[_,N]),B`
    ...
    `}function kj({mediaId:_,onPreview:$}){let[j,Z]=v(null);if(b(()=>{v2(_).then(Z).catch(()=>{})},[_]),!j)return null;let N=j.filename||"file",Y=j.metadata?.size,z=Y?E$(Y):"",W=G1(j.content_type,j.filename)==="unsupported"?"Details":"Preview";return B`
    ...
    `}function Pj({attachment:_,onPreview:$}){let j=Number(_?.id),[Z,N]=v(null);b(()=>{if(!Number.isFinite(j))return;v2(j).then(N).catch(()=>{});return},[j]);let Y=Z?.filename||_.label||`attachment-${_.id}`,z=Number.isFinite(j)?J$(j):null,W=G1(Z?.content_type,Z?.filename||_?.label)==="unsupported"?"Details":"Preview";return B`
    ...
    `}function Mj({block:_}){let $=_.title||_.name||_.uri,j=_.description,Z=_.size?E$(_.size):"",N=_.mime_type||"",Y=xj(N),z=P2(_.uri);return B`
        <a
            href=${z||"#"}
            class="resource-link"
    ...
    `}function Ij({block:_}){let[$,j]=v(!1),Z=_.uri||"Embedded resource",N=_.text||"",Y=Boolean(_.data),z=_.mime_type||"";return B`
            ...
            ${$&&B`
                ...
                ${Y&&B`
                    <div class="resource-embed-blob">
                        ...
                        <button class="resource-embed-blob-btn" onClick=${(G)=>{G.preventDefault(),G.stopPropagation();let W=new Blob([Uint8Array.from(atob(_.data),(X)=>X.charCodeAt(0))],{type:z||"application/octet-stream"}),Q=URL.createObjectURL(W),L=document.createElement("a");L.href=Q,L.download=Z.split("/").pop()||"resource",L.click(),URL.revokeObjectURL(Q)}}>Download</button>
                    </div>
                `}
            `}
        </div>
    `}function Tj({block:_,post:$,onOpenWidget:j}){if(!_)return null;let Z=d4(_,$),N=K6(_),Y=Z?.artifact?.kind||_?.artifact?.kind||_?.kind||null,z=Z?.title||_.title||_.name||"Generated widget",G=Z?.description||_.description||_.subtitle||"",W=_.open_label||"Open widget",Q=(L)=>{if(L.preventDefault(),L.stopPropagation(),!Z)return;j?.(Z)};return B`
    ...
    `}function xj(_){if(!_)return"\uD83D\uDCCE";if(_.startsWith("image/"))return"\uD83D\uDDBC️";if(_.startsWith("audio/"))return"\uD83C\uDFB5";if(_.startsWith("video/"))return"\uD83C\uDFAC";if(_.includes("pdf"))return"\uD83D\uDCC4";if(_.includes("zip")||_.includes("gzip"))return"\uD83D\uDDDC️";if(_.startsWith("text/"))return"\uD83D\uDCC4";return"\uD83D\uDCCE"}function wj({preview:_}){let $=P2(_.url),j=P2(_.image,{allowDataImage:!0}),Z=j?`background-image: url('${j}')`:"",N=_.site_name;if(!N&&$)try{N=new URL($).hostname}catch{N=$}return B`
        <a
            href=${$||"#"}
    ...
    `}function fj(_,$){return typeof _==="string"?_:""}var Cj=1800,bj=`
    ...
    </svg>`;async function vj(_){let $=typeof _==="string"?_:"";if(!$)return!1;if(navigator.clipboard?.writeText)try{return await navigator.clipboard.writeText($),!0}catch{}try{let j=document.createElement("textarea");j.value=$,j.setAttribute("readonly",""),j.style.position="fixed",j.style.opacity="0",j.style.pointerEvents="none",document.body.appendChild(j),j.select(),j.setSelectionRange(0,j.value.length);let Z=document.execCommand("copy");return document.body.removeChild(j),Z}catch{return!1}}function uj(_){if(!_)return()=>{};let $=Array.from(_.querySelectorAll("pre")).filter((Y)=>Y.querySelector("code"));if($.length===0)return()=>{};let j=new Map,Z=[],N=(Y,z)=>{let G=z||"idle";if(Y.dataset.copyState=G,G==="success")Y.innerHTML=Sj,Y.setAttribute("aria-label","Copied"),Y.setAttribute("title","Copied"),Y.classList.add("is-success"),Y.classList.remove("is-error");else if(G==="error")Y.innerHTML=Rj,Y.setAttribute("aria-label","Copy failed"),Y.setAttribute("title","Copy failed"),Y.classList.add("is-error"),Y.classList.remove("is-success");else Y.innerHTML=bj,Y.setAttribute("aria-label","Copy code"),Y.setAttribute("title","Copy code"),Y.classList.remove("is-success","is-error")};return $.forEach((Y)=>{let z=document.createElement("div");z.className="post-code-block",Y.parentNode?.insertBefore(z,Y),z.appendChild(Y);let G=document.createElement("button");G.type="button",G.className="post-code-copy-btn",N(G,"idle"),z.appendChild(G);let W=async(Q)=>{Q.preventDefault(),Q.stopPropagation();let X=Y.querySelector("code")?.textContent||"",F=await vj(X);N(G,F?"success":"error");let P=j.get(G);if(P)clearTimeout(P);let C=setTimeout(()=>{N(G,"idle"),j.delete(G)},Cj);j.set(G,C)};G.addEventListener("click",W),Z.push(()=>{G.removeEventListener("click",W);let Q=j.get(G);if(Q)clearTimeout(Q);if(z.parentNode)z.parentNode.insertBefore(Y,z),z.remove()})}),()=>{Z.forEach((Y)=>Y())}}function mj(_){if(!_)return{content:_,fileRefs:[]};let j=_.replace(/\r\n/g,`
`).replace(/\r/g,`
`).split(`
`),Z=-1;for(let Q=0;Q<j.length;Q+=1)if(j[Q].trim()==="Files:"&&j[Q+1]&&/^\s*-\s+/.test(j[Q+1])){Z=Q;break}if(Z===-1)return{content:_,fileRefs:[]};let N=[],Y=Z+1;for(;Y<j.length;Y+=1){let Q=j[Y];if(/^\s*-\s+/.test(Q))N.push(Q.replace(/^\s*-\s+/,"").trim());else if(!Q.trim())break;else break}if(N.length===0)return{content:_,fileRefs:[]};let z=j.slice(0,Z),G=j.slice(Y),W=[...z,...G].join(`
`);return W=W.replace(/\n{3,}/g,`

`).trim(),{content:W,fileRefs:N}}function gj(_){if(!_)return{content:_,messageRefs:[]};let j=_.replace(/\r\n/g,`
`).replace(/\r/g,`
`).split(`
`),Z=-1;for(let Q=0;Q<j.length;Q+=1)if(j[Q].trim()==="Referenced messages:"&&j[Q+1]&&/^\s*-\s+/.test(j[Q+1])){Z=Q;break}if(Z===-1)return{content:_,messageRefs:[]};let N=[],Y=Z+1;for(;Y<j.length;Y+=1){let Q=j[Y];if(/^\s*-\s+/.test(Q)){let X=Q.replace(/^\s*-\s+/,"").trim().match(/^message:(\S+)$/i);if(X)N.push(X[1])}else if(!Q.trim())break;else break}if(N.length===0)return{content:_,messageRefs:[]};let z=j.slice(0,Z),G=j.slice(Y),W=[...z,...G].join(`
`);return W=W.replace(/\n{3,}/g,`

`).trim(),{content:W,messageRefs:N}}function pj(_){if(!_)return{content:_,attachments:[]};let j=_.replace(/\r\n/g,`
`).replace(/\r/g,`
`).split(`
`),Z=-1;for(let Q=0;Q<j.length;Q+=1){let L=j[Q].trim();if((L==="Images:"||L==="Attachments:")&&j[Q+1]&&/^\s*-\s+/.test(j[Q+1])){Z=Q;break}}if(Z===-1)return{content:_,attachments:[]};let N=[],Y=Z+1;for(;Y<j.length;Y+=1){let Q=j[Y];if(/^\s*-\s+/.test(Q)){let L=Q.replace(/^\s*-\s+/,"").trim(),X=L.match(/^attachment:([^\s)]+)\s*(?:\((.+)\))?$/i)||L.match(/^attachment:([^\s]+)\s+(.+)$/i);if(X){let F=X[1],P=(X[2]||"").trim()||F;N.push({id:F,label:P,raw:L})}else N.push({id:null,label:L,raw:L})}else if(!Q.trim())break;else break}if(N.length===0)return{content:_,attachments:[]};let z=j.slice(0,Z),G=j.slice(Y),W=[...z,...G].join(`
`);return W=W.replace(/\n{3,}/g,`

`).trim(),{content:W,attachments:N}}function cj(_){return _.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}function hj(_,$){if(!_||!$)return _;let j=String($).trim().split(/\s+/).filter(Boolean);if(j.length===0)return _;let Z=j.map(cj).sort((L,X)=>X.length-L.length),N=new RegExp(`(${Z.join("|")})`,"gi"),Y=new RegExp(`^(${Z.join("|")})$`,"i"),z=new DOMParser().parseFromString(_,"text/html"),G=z.createTreeWalker(z.body,NodeFilter.SHOW_TEXT),W=[],Q;while(Q=G.nextNode())W.push(Q);for(let L of W){let X=L.nodeValue;if(!X||!N.test(X)){N.lastIndex=0;continue}N.lastIndex=0;let F=L.parentElement;if(F&&F.closest("code, pre, script, style"))continue;let P=X.split(N).filter((g)=>g!=="");if(P.length===0)continue;let C=z.createDocumentFragment();for(let g of P)if(Y.test(g)){let u=z.createElement("mark");u.className="search-highlight-term",u.textContent=g,C.appendChild(u)}else C.appendChild(z.createTextNode(g));L.parentNode.replaceChild(C,L)}return z.body.innerHTML}function R6({post:_,onClick:$,onHashtagClick:j,onMessageRef:Z,onScrollToMessage:N,agentName:Y,agentAvatarUrl:z,userName:G,userAvatarUrl:W,userAvatarBackground:Q,onDelete:L,isThreadReply:X,isThreadPrev:F,isThreadNext:P,isRemoving:C,highlightQuery:g,onFileRef:u,onOpenWidget:E}){let[x,m]=v(null),A=k(null),S=_.data,a=S.type==="agent_response",Z_=G||"You",N_=a?Y||B6:Z_,e=a?s4(Y,z,!0):s4(Z_,W),r=typeof Q==="string"?Q.trim().toLowerCase():"",z_=!a&&e.image&&(r==="clear"||r==="transparent"),f0=a&&Boolean(e.image),y_=`background-color: ${z_||f0?"transparent":e.color}`,$_=S.content_meta,Y_=Boolean($_?.truncated),V_=Boolean($_?.preview),X_=Y_&&!V_,U_=Y_?{originalLength:Number.isFinite($_?.original_length)?$_.original_length:S.content?S.content.length:0,maxLength:Number.isFinite($_?.max_length)?$_.max_length:0}:null,W_=S.content_blocks||[],k_=S.media_ids||[],G_=fj(S.content,S.link_previews),{content:d_,fileRefs:l_}=mj(G_),{content:V0,messageRefs:T_}=gj(d_),{content:$0,attachments:g_}=pj(V0);G_=$0;let C0=t4(W_),d=e4(W_),x_=C0.length===1&&typeof C0[0]?.fallback_text==="string"?C0[0].fallback_text.trim():"",Q_=d.length===1?f6(d[0]).trim():"",D_=Boolean(x_)&&G_?.trim()===x_||Boolean(Q_)&&G_?.trim()===Q_,R_=Boolean(G_)&&!X_&&!D_,G0=typeof g==="string"?g.trim():"",n_=I_(()=>{if(!G_||D_)return"";let w=K$(G_,j);return G0?hj(w,G0):w},[G_,D_,G0]),s_=(w,s)=>{w.stopPropagation(),m(J$(s))},[p_,o_]=v(null),w_=(w)=>{o_(w)},b_=(w)=>{w.stopPropagation(),L?.(_)},c_=(w,s)=>{let F_=new Set;if(!w||s.length===0)return{content:w,usedIds:F_};return{content:w.replace(/attachment:([^\s)"']+)/g,(j0,i0,y$,V$)=>{let A0=i0.replace(/^\/+/,""),A_=s.find((b0)=>b0.name&&b0.name.toLowerCase()===A0.toLowerCase()&&!F_.has(b0.id))||s.find((b0)=>!F_.has(b0.id));if(!A_)return j0;if(F_.add(A_.id),V$.slice(Math.max(0,y$-2),y$)==="](")return`/media/${A_.id}`;return A_.name||"attachment"}),usedIds:F_}},x0=[],J0=[],r_=[],Q0=[],X0=[],S_=[],E0=[],w0=0;if(W_.length>0)W_.forEach((w)=>{if(w?.type==="text"&&w.annotations)E0.push(w.annotations);if(w?.type==="generated_widget")S_.push(w);else if(w?.type==="resource_link")Q0.push(w);else if(w?.type==="resource")X0.push(w);else if(w?.type==="file"){let s=k_[w0++];if(s)J0.push(s),r_.push({id:s,name:w?.name||w?.filename||w?.title})}else if(w?.type==="image"||!w?.type){let s=k_[w0++];if(s){let F_=typeof w?.mime_type==="string"?w.mime_type:void 0;x0.push({id:s,annotations:w?.annotations,mimeType:F_}),r_.push({id:s,name:w?.name||w?.filename||w?.title})}}});else if(k_.length>0){let w=g_.length>0;k_.forEach((s,F_)=>{let M_=g_[F_]||null;if(r_.push({id:s,name:M_?.label||null}),w)J0.push(s);else x0.push({id:s,annotations:null})})}if(g_.length>0)g_.forEach((w)=>{if(!w?.id)return;let s=r_.find((F_)=>String(F_.id)===String(w.id));if(s&&!s.name)s.name=w.label});let{content:P_,usedIds:Y0}=c_(G_,r_);G_=P_;let W0=x0.filter(({id:w})=>!Y0.has(w)),E_=J0.filter((w)=>!Y0.has(w)),L0=g_.length>0?g_.map((w,s)=>({id:w.id||`attachment-${s+1}`,label:w.label||`attachment-${s+1}`})):r_.map((w,s)=>({id:w.id,label:w.name||`attachment-${s+1}`})),f_=I_(()=>t4(W_),[W_]),C_=I_(()=>e4(W_),[W_]),v_=I_(()=>{return f_.map((w)=>`${w.card_id}:${w.state}`).join("|")},[f_]);b(()=>{if(!A.current)return;return j2(A.current),uj(A.current)},[n_]);let e0=k(null);return b(()=>{if(!e0.current||f_.length===0)return;let w=e0.current;w.innerHTML="";for(let s of f_){let F_=document.createElement("div");w.appendChild(F_),x6(F_,s,{onAction:async(M_)=>{if(M_.type==="Action.OpenUrl"){let j0=P2(M_.url||"");if(!j0)throw Error("Invalid URL");window.open(j0,"_blank","noopener,noreferrer");return}if(M_.type==="Action.Submit"){await y4({post_id:_.id,thread_id:S.thread_id||_.id,chat_jid:_.chat_jid||null,card_id:s.card_id,action:{type:M_.type,title:M_.title||"",data:M_.data}});return}console.warn("[post] unsupported adaptive card action:",M_.type,M_)}}).catch((M_)=>{console.error("[post] adaptive card render error:",M_),F_.textContent=s.fallback_text||"Card failed to render."})}},[v_,_.id]),B`
        <div id=${`post-${_.id}`} class="post ${a?"agent-post":""} ${X?"thread-reply":""} ${F?"thread-prev":""} ${P?"thread-next":""} ${C?"removing":""}" onClick=${$}>
            ...
            <div class="post-body">
                ...
                `}
                ${(l_.length>0||T_.length>0||L0.length>0)&&B`
                ...
                ${W0.length>0&&B`
                    <div class="media-preview">
                        ${W0.map(({id:w,mimeType:s})=>{let M_=typeof s==="string"&&s.toLowerCase().startsWith("image/svg")?J$(w):I4(w);return B`
                                <img 
                                    key=${w} 
                                    src=${M_} 
                                    alt="Media" 
                ...
                ${E_.length>0&&B`
                    <div class="file-attachments">
                        ${E_.map((w)=>B`
                            <${kj} key=${w} mediaId=${w} onPreview=${w_} />
                        `)}
                    </div>
                `}
                ${Q0.length>0&&B`
        ...
        ${p_&&B`
            ...
            />
        `}
    `}function v6({posts:_,hasMore:$,onLoadMore:j,onPostClick:Z,onHashtagClick:N,onMessageRef:Y,onScrollToMessage:z,onFileRef:G,onOpenWidget:W,emptyMessage:Q,timelineRef:L,agents:X,user:F,onDeletePost:P,reverse:C=!0,removingPostIds:g,searchQuery:u}){let[E,x]=v(!1),m=k(null),A=typeof IntersectionObserver<"u",S=M(async()=>{if(!j||!$||E)return;x(!0);try{await j({preserveScroll:!0,preserveMode:"top"})}finally{x(!1)}},[$,E,j]),a=M(($_)=>{let{scrollTop:Y_,scrollHeight:V_,clientHeight:X_}=$_.target,U_=C?V_-X_-Y_:Y_,W_=Math.max(300,X_);if(U_<W_)S()},[C,S]);b(()=>{if(!A)return;let $_=m.current,Y_=L?.current;if(!$_||!Y_)return;let V_=300,X_=new IntersectionObserver((U_)=>{for(let W_ of U_){if(!W_.isIntersecting)continue;S()}},{root:Y_,rootMargin:`${V_}px 0px ${V_}px 0px`,threshold:0});return X_.observe($_),()=>X_.disconnect()},[A,$,j,L,S]);let Z_=k(S);if(Z_.current=S,b(()=>{if(A)return;if(!L?.current)return;let{scrollTop:$_,scrollHeight:Y_,clientHeight:V_}=L.current,X_=C?Y_-V_-$_:$_,U_=Math.max(300,V_);if(X_<U_)Z_.current?.()},[A,_,$,C,L]),b(()=>{if(!L?.current)return;if(!$||E)return;let{scrollTop:$_,scrollHeight:Y_,clientHeight:V_}=L.current,X_=C?Y_-V_-$_:$_,U_=Math.max(300,V_);if(Y_<=V_+1||X_<U_)Z_.current?.()},[_,$,E,C,L]),!_)return B`<div class="loading"><div class="spinner"></div></div>`;if(_.length===0)return B`
        ...
        <div class="timeline ${C?"reverse":"normal"}" ref=${L} onScroll=${a}>
            <div class="timeline-content">
                ${C?y_:null}
...
`),this.setStatus("Unavailable")}}sendResize(){if(!this.socket||this.socket.readyState!==WebSocket.OPEN||!this.fitAddon?.proposeDimensions)return;let _=this.fitAddon.proposeDimensions();if(!_)return;this.socket.send(JSON.stringify({type:"resize",cols:_.cols,rows:_.rows}))}getContent(){return}isDirty(){return!1}focus(){if(this.terminal?.focus){this.terminal.focus();return}this.termEl?.focus()}resize(){this.syncHostLayout();try{this.terminal?.renderer?.remeasureFont?.()}catch{}try{this.fitAddon?.fit?.()}catch{}try{this.terminal?.refresh?.()}catch{}this.syncHostLayout(),this.sendResize()}dispose(){if(this.disposed)return;this.disposed=!0;try{if(this.resizeFrame)cancelAnimationFrame(this.resizeFrame),this.resizeFrame=0}catch{}try{if(this.themeChangeListener)window.removeEventListener("piclaw-theme-change",this.themeChangeListener)}catch{}try{if(this.mediaQuery&&this.mediaQueryListener){if(this.mediaQuery.removeEventListener)this.mediaQuery.removeEventListener("change",this.mediaQueryListener);else if(this.mediaQuery.removeListener)this.mediaQuery.removeListener(this.mediaQueryListener)}}catch{}try{if(this.dockResizeListener)window.removeEventListener("dock-resize",this.dockResizeListener);if(this.windowResizeListener)window.removeEventListener("resize",this.windowResizeListener)}catch{}try{this.themeObserver?.disconnect?.()}catch{}try{this.resizeObserver?.disconnect?.()}catch{}try{this.socket?.close?.()}catch{}try{this.fitAddon?.dispose?.()}catch{}try{this.terminal?.dispose?.()}catch{}this.termEl?.remove()}}var z3={id:"terminal",label:"Terminal",icon:"terminal",capabilities:["terminal"],placement:"dock",mount(_,$){return new Y3(_,$)}},K3={id:"terminal-tab",label:"Terminal",icon:"terminal",capabilities:["terminal"],placement:"tabs",canHandle(_){return _?.path==="piclaw://terminal"?1e4:!1},mount(_,$){return new Y3(_,$)}};function W2(_){return String(_||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function $Z(_,$){let j=String(_||"").trim();if(!j)return j;if(/^[a-zA-Z][a-zA-Z\d+.-]*:/.test(j)||j.startsWith("#")||j.startsWith("data:")||j.startsWith("blob:"))return j;let Z=j.match(/^([^?#]*)(\?[^#]*)?(#.*)?$/),N=Z?.[1]||j,Y=Z?.[2]||"",z=Z?.[3]||"",G=String($||"").split("/").slice(0,-1).join("/"),Q=N.startsWith("/")?N:`${G?`${G}/`:""}${N}`,L=[];for(let F of Q.split("/")){if(!F||F===".")continue;if(F===".."){if(L.length>0)L.pop();continue}L.push(F)}let X=L.join("/");return`${M1(X)}${Y}${z}`}function q1(_){return _?.preview||null}function jZ(_){let $=String(_||""),j=Math.max($.lastIndexOf("/"),$.lastIndexOf("\\")),Z=j>=0?$.slice(j+1):$,N=Z.lastIndexOf(".");if(N<=0||N===Z.length-1)return"none";return Z.slice(N+1)}function ZZ(_){if(!_)return"unknown";if(_.kind==="image")return"image";if(_.kind==="text")return _.content_type==="text/markdown"?"markdown":"text";if(_.kind==="binary")return"binary";return String(_.kind||"unknown")}function NZ(_,$){let j=$?.path||_?.path||"",Z=[];if($?.content_type)Z.push(`<span><strong>type:</strong> ${W2($.content_type)}</span>`);if(typeof $?.size==="number")Z.push(`<span><strong>size:</strong> ${W2(E$($.size))}</span>`);if($?.mtime)Z.push(`<span><strong>modified:</strong> ${W2(M2($.mtime))}</span>`);if(Z.push(`<span><strong>kind:</strong> ${W2(ZZ($))}</span>`),Z.push(`<span><strong>extension:</strong> ${W2(jZ(j))}</span>`),j)Z.push(`<span><strong>path:</strong> ${W2(j)}</span>`);if($?.truncated)Z.push("<span><strong>content:</strong> truncated</span>");return`<div class="workspace-preview-meta workspace-preview-meta-inline">${Z.join("")}</div>`}function YZ(_){let $=q1(_);if(!$)return'<div class="workspace-preview-text">No preview available.</div>';let j=NZ(_,$);if($.kind==="image"){let Z=$.url||($.path?M1($.path):"");return`${j}
        ...
        `,_.appendChild(N);let Y=N.querySelector("#video-open-tab");if(Y)Y.addEventListener("click",()=>{let z=new CustomEvent("video-viewer:open-tab",{bubbles:!0,detail:{path:j}});_.dispatchEvent(z)})}getContent(){return}isDirty(){return!1}focus(){}resize(){}dispose(){if(this.disposed)return;this.disposed=!0,this.container.innerHTML=""}}class _9{container;iframe=null;disposed=!1;constructor(_,$){this.container=_;let j=$.path||"",Z=`/video-viewer/?path=${encodeURIComponent(j)}`;this.iframe=document.createElement("iframe"),this.iframe.src=Z,this.iframe.style.cssText="width:100%;height:100%;border:none;background:#111;",_.appendChild(this.iframe)}getContent(){return}isDirty(){return!1}focus(){this.iframe?.focus()}resize(){}dispose(){if(this.disposed)return;if(this.disposed=!0,this.iframe)this.iframe.src="about:blank",this.iframe=null;this.container.innerHTML=""}}var U3={id:"video-viewer",label:"Video Viewer",icon:"play-circle",capabilities:["readonly","preview"],placement:"tabs",canHandle(_){let $=_?.path||"";if(!WZ.test($))return!1;return 54},mount(_,$){if($?.mode==="view")return new e6(_,$);return new _9(_,$)}};function BZ(_){if(!_)return!1;let $=_.toLowerCase();return $.endsWith(".drawio")||$.endsWith(".drawio.xml")||$.endsWith(".drawio.svg")||$.endsWith(".drawio.png")}function UZ(_){return _.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}var O3='<mxfile host="app.diagrams.net"><diagram id="page-1" name="Page-1"><mxGraphModel dx="1260" dy="720" grid="1" gridSize="10" guides="1" tooltips="1" connect="1" arrows="1" fold="1" page="1" pageScale="1" pageWidth="850" pageHeight="1100" math="0" shadow="0"><root><mxCell id="0"/><mxCell id="1" parent="0"/></root></mxGraphModel></diagram></mxfile>';function $9(_){let $=String(_||"").trim();return $?$:O3}function OZ(_){let $=String(_||"").toLowerCase();if($.endsWith(".drawio.svg")||$.endsWith(".svg"))return"xmlsvg";if($.endsWith(".drawio.png")||$.endsWith(".png"))return"xmlpng";return"xml"}function FZ(_){let $="",j=32768;for(let Z=0;Z<_.length;Z+=j)$+=String.fromCharCode(..._.subarray(Z,Z+j));return btoa($)}function HZ(_,$="*"){try{let j=(Y)=>{let z=_.parent||_.opener;if(!z)return!1;return z.postMessage(JSON.stringify({event:"workspace-export",...Y}),$),!0},Z=_.EditorUi;if(Z?.prototype&&!Z.prototype.__piclawWorkspaceSavePatched){let Y=Z.prototype.saveData;Z.prototype.saveData=function(z,G,W,Q,L,X){try{if(z&&W!=null&&j({filename:z,format:G,data:W,mimeType:Q,base64Encoded:Boolean(L),defaultMode:X}))return}catch(F){console.warn("[drawio-pane] saveData intercept failed, falling back to native save",F)}return Y.apply(this,arguments)},Z.prototype.__piclawWorkspaceSavePatched=!0}let N=_.App;if(N?.prototype&&!N.prototype.__piclawExportPatched){let Y=N.prototype.exportFile;N.prototype.exportFile=function(z,G,W,Q,L,X){try{if(G&&j({filename:G,data:z,mimeType:W,base64Encoded:Boolean(Q),mode:L,folderId:X}))return}catch(F){console.warn("[drawio-pane] export intercept failed, falling back to native export",F)}return Y.apply(this,arguments)},N.prototype.__piclawExportPatched=!0}return Boolean(Z?.prototype&&Z.prototype.__piclawWorkspaceSavePatched||N?.prototype&&N.prototype.__piclawExportPatched)}catch{return!1}}async function j9(_,$){let j=new Uint8Array(await _.arrayBuffer());return`data:${_.headers.get("Content-Type")||$};base64,${FZ(j)}`}class Z9{container;disposed=!1;constructor(_,$){this.container=_;let j=$.path||"",Z=j.split("/").pop()||"diagram.drawio",N=document.createElement("div");N.style.cssText="width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:var(--bg-primary,#1a1a1a);",N.innerHTML=`
            <div style="text-align:center;max-width:360px;padding:24px;">
                <div style="font-size:56px;margin-bottom:12px;">\uD83D\uDCD0</div>
                <div style="font-size:14px;font-weight:600;color:var(--text-primary,#e0e0e0);margin-bottom:4px;word-break:break-word;">${UZ(Z)}</div>
        ...
        `,_.appendChild(N);let Y=N.querySelector("#drawio-open-tab");if(Y)Y.addEventListener("click",()=>{let z=new CustomEvent("drawio:open-tab",{bubbles:!0,detail:{path:j}});_.dispatchEvent(z)})}getContent(){return}isDirty(){return!1}focus(){}resize(){}dispose(){if(this.disposed)return;this.disposed=!0,this.container.innerHTML=""}}class N9{container;iframe=null;overlay=null;disposed=!1;filePath;fileName;format;xmlData="";fileLoaded=!1;editorReady=!1;loadSent=!1;saveChain=Promise.resolve();onMessageBound;constructor(_,$){this.container=_,this.filePath=$.path||"",this.fileName=this.filePath.split("/").pop()||"diagram.drawio",this.format=OZ(this.filePath),this.onMessageBound=this.onMessage.bind(this);let j=document.createElement("div");j.style.cssText="position:relative;width:100%;height:100%;background:#1e1e1e;",this.overlay=document.createElement("div"),this.overlay.style.cssText="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;color:#999;font:14px system-ui,sans-serif;z-index:1;pointer-events:none;",this.overlay.textContent="Loading draw.io editor…",j.appendChild(this.overlay);let N=`/drawio/index.html?embed=1&proto=json&spin=1&modified=0&noSaveBtn=1&noExitBtn=1&saveAndExit=0&ui=dark&dark=${window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches?"1":"0"}`;this.iframe=document.createElement("iframe"),this.iframe.src=N,this.iframe.style.cssText="width:100%;height:100%;border:none;background:#1e1e1e;position:relative;z-index:0;",this.iframe.addEventListener("load",()=>{let Y=()=>{if(!this.iframe?.contentWindow||this.disposed)return;if(HZ(this.iframe.contentWindow))return;setTimeout(Y,250)};Y()}),j.appendChild(this.iframe),_.appendChild(j),window.addEventListener("message",this.onMessageBound),this.loadFile()}async loadFile(){if(!this.filePath){this.xmlData=O3,this.fileLoaded=!0,this.trySendLoad();return}try{let _=await fetch(`/workspace/raw?path=${encodeURIComponent(this.filePath)}`);if(_.ok)if(this.format==="xmlsvg")this.xmlData=await j9(_,"image/svg+xml");else if(this.format==="xmlpng")this.xmlData=await j9(_,"image/png");else this.xmlData=$9(await _.text());else if(_.status===404)this.xmlData=O3;else throw Error(`HTTP ${_.status}`);this.fileLoaded=!0,this.trySendLoad()}catch(_){if(this.overlay)this.overlay.textContent=`Failed to load: ${_ instanceof Error?_.message:String(_)}`}}trySendLoad(){if(this.disposed||this.loadSent||!this.editorReady||!this.fileLoaded||!this.iframe?.contentWindow)return;if(this.loadSent=!0,this.iframe.contentWindow.postMessage(JSON.stringify({action:"load",xml:this.format==="xml"?$9(this.xmlData):this.xmlData,autosave:1,saveAndExit:"0",noSaveBtn:"1",noExitBtn:"1",title:this.fileName}),"*"),this.overlay)this.overlay.style.display="none"}queueSave(_,$){if(!this.filePath)return;this.saveChain=this.saveChain.then(async()=>{let j=await fetch("/drawio/save",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({path:this.filePath,format:_.format||this.format,xml:_.xml,data:_.data,mimeType:_.mimeType,filename:_.filename,base64Encoded:_.base64Encoded})});if(!j.ok)throw Error(`HTTP ${j.status}`);if($&&this.iframe?.contentWindow)this.iframe.contentWindow.postMessage(JSON.stringify({action:"status",message:"Saved",modified:!1}),"*")}).catch((j)=>{if(console.error("[drawio-pane] save failed:",j),this.overlay)this.overlay.style.display="flex",this.overlay.textContent=`Save failed: ${j instanceof Error?j.message:String(j)}`})}onMessage(_){if(this.disposed||_.source!==this.iframe?.contentWindow)return;let $;try{$=typeof _.data==="string"?JSON.parse(_.data):_.data}catch{return}switch($?.event){case"init":this.editorReady=!0,this.trySendLoad();break;case"autosave":if(this.format==="xml"){if(typeof $.xml==="string")this.queueSave({xml:$.xml,format:"xml"},!1)}else if(typeof $.xml==="string")this.xmlData=$.xml;break;case"save":if(this.format==="xml"){if(typeof $.xml==="string")this.queueSave({xml:$.xml,format:"xml"},!0)}else if(typeof $.xml==="string"&&this.iframe?.contentWindow)this.xmlData=$.xml,this.iframe.contentWindow.postMessage(JSON.stringify({action:"export",format:this.format,xml:$.xml,spinKey:"export"}),"*");break;case"export":if(typeof $.data==="string")this.queueSave({data:$.data,format:this.format,xml:typeof $.xml==="string"?$.xml:void 0},!0);break;case"workspace-export":if(typeof $.data==="string")this.queueSave({data:$.data,xml:typeof $.xml==="string"?$.xml:void 0,mimeType:typeof $.mimeType==="string"?$.mimeType:void 0,filename:typeof $.filename==="string"?$.filename:void 0,base64Encoded:Boolean($.base64Encoded),format:this.format},!0);break;case"exit":default:break}}getContent(){return}isDirty(){return!1}focus(){this.iframe?.focus()}resize(){}dispose(){if(this.disposed)return;if(this.disposed=!0,window.removeEventListener("message",this.onMessageBound),this.iframe)this.iframe.src="about:blank",this.iframe=null;this.overlay=null,this.container.innerHTML=""}}var F3={id:"drawio-editor",label:"Draw.io Editor",icon:"git-merge",capabilities:["edit","preview"],placement:"tabs",canHandle(_){if(!BZ(_?.path))return!1;return 60},mount(_,$){if($?.mode==="view")return new Z9(_,$);return new N9(_,$)}};class Y9{tabs=new Map;activeId=null;mruOrder=[];listeners=new Set;onChange(_){return this.listeners.add(_),()=>this.listeners.delete(_)}notify(){let _=this.getTabs(),$=this.activeId;for(let j of this.listeners)try{j(_,$)}catch{}}open(_,$){let j=this.tabs.get(_);if(!j)j={id:_,label:$||_.split("/").pop()||_,path:_,dirty:!1,pinned:!1},this.tabs.set(_,j);return this.activate(_),j}activate(_){if(!this.tabs.has(_))return;this.activeId=_,this.mruOrder=[_,...this.mruOrder.filter(($)=>$!==_)],this.notify()}close(_){if(!this.tabs.get(_))return!1;if(this.tabs.delete(_),this.mruOrder=this.mruOrder.filter((j)=>j!==_),this.activeId===_)this.activeId=this.mruOrder[0]||null;return this.notify(),!0}closeOthers(_){for(let[$,j]of this.tabs)if($!==_&&!j.pinned)this.tabs.delete($),this.mruOrder=this.mruOrder.filter((Z)=>Z!==$);if(this.activeId&&!this.tabs.has(this.activeId))this.activeId=_;this.notify()}closeAll(){for(let[_,$]of this.tabs)if(!$.pinned)this.tabs.delete(_),this.mruOrder=this.mruOrder.filter((j)=>j!==_);if(this.activeId&&!this.tabs.has(this.activeId))this.activeId=this.mruOrder[0]||null;this.notify()}setDirty(_,$){let j=this.tabs.get(_);if(!j||j.dirty===$)return;j.dirty=$,this.notify()}togglePin(_){let $=this.tabs.get(_);if(!$)return;$.pinned=!$.pinned,this.notify()}saveViewState(_,$){let j=this.tabs.get(_);if(j)j.viewState=$}getViewState(_){return this.tabs.get(_)?.viewState}rename(_,$,j){let Z=this.tabs.get(_);if(!Z)return;if(this.tabs.delete(_),Z.id=$,Z.path=$,Z.label=j||$.split("/").pop()||$,this.tabs.set($,Z),this.mruOrder=this.mruOrder.map((N)=>N===_?$:N),this.activeId===_)this.activeId=$;this.notify()}getTabs(){return Array.from(this.tabs.values())}getActiveId(){return this.activeId}getActive(){return this.activeId?this.tabs.get(this.activeId)||null:null}get(_){return this.tabs.get(_)}get size(){return this.tabs.size}hasUnsaved(){for(let _ of this.tabs.values())if(_.dirty)return!0;return!1}getDirtyTabs(){return Array.from(this.tabs.values()).filter((_)=>_.dirty)}nextTab(){let _=this.getTabs();if(_.length<=1)return;let $=_.findIndex((Z)=>Z.id===this.activeId),j=_[($+1)%_.length];this.activate(j.id)}prevTab(){let _=this.getTabs();if(_.length<=1)return;let $=_.findIndex((Z)=>Z.id===this.activeId),j=_[($-1+_.length)%_.length];this.activate(j.id)}mruSwitch(){if(this.mruOrder.length>1)this.activate(this.mruOrder[1])}}var _0=new Y9;var n1="workspaceExplorerScale",DZ=["compact","default","comfortable"],JZ=new Set(DZ),EZ={compact:{indentPx:14},default:{indentPx:16},comfortable:{indentPx:18}};function z9(_,$="default"){if(typeof _!=="string")return $;let j=_.trim().toLowerCase();return JZ.has(j)?j:$}function H3(){if(typeof window>"u")return{width:0,isTouch:!1};let _=Number(window.innerWidth)||0,$=Boolean(window.matchMedia?.("(pointer: coarse)")?.matches),j=Boolean(window.matchMedia?.("(hover: none)")?.matches),Z=Number(globalThis.navigator?.maxTouchPoints||0)>0;return{width:_,isTouch:$||Z&&j}}function AZ(_={}){let $=Math.max(0,Number(_.width)||0);if(Boolean(_.isTouch))return"comfortable";if($>0&&$<1180)return"comfortable";return"default"}function yZ(_,$={}){if(Boolean($.isTouch)&&_==="compact")return"default";return _}function D3(_={}){let $=AZ(_),j=_.stored?z9(_.stored,$):$;return yZ(j,_)}function K9(_){return EZ[z9(_)]}var kZ=60000,q9=(_)=>{if(!_||!_.name)return!1;if(_.path===".")return!1;return _.name.startsWith(".")},PZ=[".md",".markdown",".txt",".ts",".tsx",".js",".jsx",".json",".jsonl",".css",".scss",".html",".htm",".yaml",".yml",".sql",".sqlite",".xml",".svg",".plist",".sh",".bash",".zsh",".py",".go",".toml",".ini",".conf",".cfg",".env",".gitignore",".dockerignore",".editorconfig"];function MZ(_){let $=String(_||"").trim().toLowerCase();if(!$||$.endsWith("/"))return!1;return PZ.some((j)=>$.endsWith(j))}function X9(_,$,j,Z=0,N=[]){if(!j&&q9(_))return N;if(!_)return N;if(N.push({node:_,depth:Z}),_.type==="dir"&&_.children&&$.has(_.path))for(let Y of _.children)X9(Y,$,j,Z+1,N);return N}function V9(_,$,j){if(!_)return"";let Z=[],N=(Y)=>{if(!j&&q9(Y))return;if(Z.push(Y.type==="dir"?`d:${Y.path}`:`f:${Y.path}`),Y.children&&$?.has(Y.path))for(let z of Y.children)N(z)};return N(_),Z.join("|")}function y3(_,$){if(!$)return null;if(!_)return $;if(_.path!==$.path||_.type!==$.type)return $;let j=Array.isArray(_.children)?_.children:null,Z=Array.isArray($.children)?$.children:null;if(!Z)return _;let N=j?new Map(j.map((G)=>[G?.path,G])):new Map,Y=!j||j.length!==Z.length,z=Z.map((G)=>{let W=y3(N.get(G.path),G);if(W!==N.get(G.path))Y=!0;return W});return Y?{...$,children:z}:_}function E3(_,$,j){if(!_)return _;if(_.path===$)return y3(_,j);if(!Array.isArray(_.children))return _;let Z=!1,N=_.children.map((Y)=>{let z=E3(Y,$,j);if(z!==Y)Z=!0;return z});return Z?{..._,children:N}:_}var W9=4,J3=14,IZ=8,TZ=16;function L9(_){if(!_)return 0;if(_.type==="file"){let Z=Math.max(0,Number(_.size)||0);return _.__bytes=Z,Z}let $=Array.isArray(_.children)?_.children:[],j=0;for(let Z of $)j+=L9(Z);return _.__bytes=j,j}function B9(_,$=0){let j=Math.max(0,Number(_?.__bytes??_?.size??0)),Z={name:_?.name||_?.path||".",path:_?.path||".",size:j,children:[]};if(!_||_.type!=="dir"||$>=W9)return Z;let N=Array.isArray(_.children)?_.children:[],Y=[];for(let G of N){let W=Math.max(0,Number(G?.__bytes??G?.size??0));if(W<=0)continue;if(G.type==="dir")Y.push({kind:"dir",node:G,size:W});else Y.push({kind:"file",name:G.name,path:G.path,size:W})}Y.sort((G,W)=>W.size-G.size);let z=Y;if(Y.length>J3){let G=Y.slice(0,J3-1),W=Y.slice(J3-1),Q=W.reduce((L,X)=>L+X.size,0);G.push({kind:"other",name:`+${W.length} more`,path:`${Z.path}/[other]`,size:Q}),z=G}return Z.children=z.map((G)=>{if(G.kind==="dir")return B9(G.node,$+1);return{name:G.name,path:G.path,size:G.size,children:[]}}),Z}function G9(){if(typeof window>"u"||typeof document>"u")return!1;let{documentElement:_,body:$}=document,j=_?.getAttribute?.("data-theme")?.toLowerCase?.()||"";if(j==="dark")return!0;if(j==="light")return!1;if(_?.classList?.contains("dark")||$?.classList?.contains("dark"))return!0;if(_?.classList?.contains("light")||$?.classList?.contains("light"))return!1;return Boolean(window.matchMedia?.("(prefers-color-scheme: dark)")?.matches)}function U9(_,$,j){let Z=((_+Math.PI/2)*180/Math.PI+360)%360,N=j?Math.max(30,70-$*10):Math.max(34,66-$*8),Y=j?Math.min(70,45+$*5):Math.min(60,42+$*4);return`hsl(${Z.toFixed(1)} ${N}% ${Y}%)`}function d1(_,$,j,Z){return{x:_+j*Math.cos(Z),y:$+j*Math.sin(Z)}}function k3(_,$,j,Z,N,Y){let z=Math.PI*2-0.0001,G=Y-N>z?N+z:Y,W=d1(_,$,Z,N),Q=d1(_,$,Z,G),L=d1(_,$,j,G),X=d1(_,$,j,N),F=G-N>Math.PI?1:0;return[`M ${W.x.toFixed(3)} ${W.y.toFixed(3)}`,`A ${Z} ${Z} 0 ${F} 1 ${Q.x.toFixed(3)} ${Q.y.toFixed(3)}`,`L ${L.x.toFixed(3)} ${L.y.toFixed(3)}`,`A ${j} ${j} 0 ${F} 0 ${X.x.toFixed(3)} ${X.y.toFixed(3)}`,"Z"].join(" ")}var O9={1:[26,46],2:[48,68],3:[70,90],4:[92,112]};function F9(_,$,j){let Z=[],N=[],Y=Math.max(0,Number($)||0),z=(G,W,Q,L)=>{let X=Array.isArray(G?.children)?G.children:[];if(!X.length)return;let F=Math.max(0,Number(G.size)||0);if(F<=0)return;let P=Q-W,C=W;X.forEach((g,u)=>{let E=Math.max(0,Number(g.size)||0);if(E<=0)return;let x=E/F,m=C,A=u===X.length-1?Q:C+P*x;if(C=A,A-m<0.003)return;let S=O9[L];if(S){let a=U9(m,L,j);if(Z.push({key:g.path,path:g.path,label:g.name,size:E,color:a,depth:L,startAngle:m,endAngle:A,innerRadius:S[0],outerRadius:S[1],d:k3(120,120,S[0],S[1],m,A)}),L===1)N.push({key:g.path,name:g.name,size:E,pct:Y>0?E/Y*100:0,color:a})}if(L<W9)z(g,m,A,L+1)})};return z(_,-Math.PI/2,Math.PI*3/2,1),{segments:Z,legend:N}}function A3(_,$){if(!_||!$)return null;if(_.path===$)return _;let j=Array.isArray(_.children)?_.children:[];for(let Z of j){let N=A3(Z,$);if(N)return N}return null}function H9(_,$,j,Z){if(!j||j<=0)return{segments:[],legend:[]};let N=O9[1];if(!N)return{segments:[],legend:[]};let Y=-Math.PI/2,z=Math.PI*3/2,G=U9(Y,1,Z),Q=`${$||"."}/[files]`;return{segments:[{key:Q,path:Q,label:_,size:j,color:G,depth:1,startAngle:Y,endAngle:z,innerRadius:N[0],outerRadius:N[1],d:k3(120,120,N[0],N[1],Y,z)}],legend:[{key:Q,name:_,size:j,pct:100,color:G}]}}function Q9(_,$=!1,j=!1){if(!_)return null;let Z=L9(_),N=B9(_,0),Y=N.size||Z,{segments:z,legend:G}=F9(N,Y,j);if(!z.length&&Y>0){let W=H9("[files]",N.path,Y,j);z=W.segments,G=W.legend}return{root:N,totalSize:Y,segments:z,legend:G,truncated:$,isDarkTheme:j}}function xZ({payload:_}){if(!_)return null;let[$,j]=v(null),[Z,N]=v(_?.root?.path||"."),[Y,z]=v(()=>[_?.root?.path||"."]),[G,W]=v(!1);b(()=>{let r=_?.root?.path||".";N(r),z([r]),j(null)},[_?.root?.path,_?.totalSize]),b(()=>{if(!Z)return;W(!0);let r=setTimeout(()=>W(!1),180);return()=>clearTimeout(r)},[Z]);let Q=I_(()=>{return A3(_.root,Z)||_.root},[_?.root,Z]),L=Q?.size||_.totalSize||0,{segments:X,legend:F}=I_(()=>{let r=F9(Q,L,_.isDarkTheme);if(r.segments.length>0)return r;if(L<=0)return r;let z_=Q?.children?.length?"Total":"[files]";return H9(z_,Q?.path||_?.root?.path||".",L,_.isDarkTheme)},[Q,L,_.isDarkTheme,_?.root?.path]),[P,C]=v(X),g=k(new Map),u=k(0);b(()=>{let r=g.current,z_=new Map(X.map((Y_)=>[Y_.key,Y_])),f0=performance.now(),y_=220,$_=(Y_)=>{let V_=Math.min(1,(Y_-f0)/220),X_=V_*(2-V_),U_=X.map((W_)=>{let G_=r.get(W_.key)||{startAngle:W_.startAngle,endAngle:W_.startAngle,innerRadius:W_.innerRadius,outerRadius:W_.innerRadius},d_=(g_,C0)=>g_+(C0-g_)*X_,l_=d_(G_.startAngle,W_.startAngle),V0=d_(G_.endAngle,W_.endAngle),T_=d_(G_.innerRadius,W_.innerRadius),$0=d_(G_.outerRadius,W_.outerRadius);return{...W_,d:k3(120,120,T_,$0,l_,V0)}});if(C(U_),V_<1)u.current=requestAnimationFrame($_)};if(u.current)cancelAnimationFrame(u.current);return u.current=requestAnimationFrame($_),g.current=z_,()=>{if(u.current)cancelAnimationFrame(u.current)}},[X]);let E=P.length?P:X,x=L>0?E$(L):"0 B",m=Q?.name||"",S=(m&&m!=="."?m:"Total")||"Total",a=x,Z_=Y.length>1,N_=(r)=>{if(!r?.path)return;let z_=A3(_.root,r.path);if(!z_||!Array.isArray(z_.children)||z_.children.length===0)return;z((f0)=>[...f0,z_.path]),N(z_.path),j(null)},e=()=>{if(!Z_)return;z((r)=>{let z_=r.slice(0,-1);return N(z_[z_.length-1]||_?.root?.path||"."),z_}),j(null)};return B`
    ...
    `}function D9({onFileSelect:_,visible:$=!0,active:j=void 0,onOpenEditor:Z,onOpenTerminalTab:N,onToggleTerminal:Y,terminalVisible:z=!1}){let[G,W]=v(null),[Q,L]=v(new Set(["."])),[X,F]=v(null),[P,C]=v(null),[g,u]=v(""),[E,x]=v(null),[m,A]=v(null),[S,a]=v(!0),[Z_,N_]=v(!1),[e,r]=v(null),[z_,f0]=v(()=>u2("workspaceShowHidden",!1)),[y_,$_]=v(!1),[Y_,V_]=v(null),[X_,U_]=v(null),[W_,k_]=v(null),[G_,d_]=v(!1),[l_,V0]=v(null),[T_,$0]=v(()=>G9()),[g_,C0]=v(()=>D3({stored:z$(n1),...H3()})),[d,x_]=v(!1),Q_=k(Q),D_=k(""),R_=k(null),G0=k(0),n_=k(new Set),s_=k(null),p_=k(new Map),o_=k(_),w_=k(Z),b_=k(null),c_=k(null),x0=k(null),J0=k(null),r_=k(null),Q0=k(null),X0=k("."),S_=k(null),E0=k({path:null,dragging:!1,startX:0,startY:0}),w0=k({path:null,dragging:!1,startX:0,startY:0}),P_=k({path:null,timer:0}),Y0=k(!1),W0=k(0),E_=k(new Map),L0=k(null),f_=k(null),C_=k(null),v_=k(null),e0=k(null),w=k(null),s=k(z_),F_=k($),M_=k(j??$),j0=k(0),i0=k(W_),y$=k(y_),V$=k(Y_),A0=k(null),_$=k({x:0,y:0}),A_=k(0),N2=k(null),b0=k(X),G$=k(P),k$=k(null),h$=k(E);o_.current=_,w_.current=Z,b(()=>{Q_.current=Q},[Q]),b(()=>{s.current=z_},[z_]),b(()=>{F_.current=$},[$]),b(()=>{M_.current=j??$},[j,$]),b(()=>{i0.current=W_},[W_]),b(()=>{if(typeof window>"u")return;let U=()=>{C0(D3({stored:z$(n1),...H3()}))};U();let H=()=>U(),T=()=>U(),I=(l)=>{if(!l||l.key===null||l.key===n1)U()};window.addEventListener("resize",H),window.addEventListener("focus",T),window.addEventListener("storage",I);let p=window.matchMedia?.("(pointer: coarse)"),t=window.matchMedia?.("(hover: none)"),j_=(l,B_)=>{if(!l)return;if(l.addEventListener)l.addEventListener("change",B_);else if(l.addListener)l.addListener(B_)},__=(l,B_)=>{if(!l)return;if(l.removeEventListener)l.removeEventListener("change",B_);else if(l.removeListener)l.removeListener(B_)};return j_(p,H),j_(t,H),()=>{window.removeEventListener("resize",H),window.removeEventListener("focus",T),window.removeEventListener("storage",I),__(p,H),__(t,H)}},[]),b(()=>{let U=(H)=>{let T=H?.detail?.path;if(!T)return;let I=T.split("/"),p=[];for(let t=1;t<I.length;t++)p.push(I.slice(0,t).join("/"));if(p.length)L((t)=>{let j_=new Set(t);j_.add(".");for(let __ of p)j_.add(__);return j_});F(T),requestAnimationFrame(()=>{let t=document.querySelector(`[data-path="${CSS.escape(T)}"]`);if(t)t.scrollIntoView({block:"nearest",behavior:"smooth"})})};return window.addEventListener("workspace-reveal-path",U),()=>window.removeEventListener("workspace-reveal-path",U)},[]),b(()=>{y$.current=y_},[y_]),b(()=>{V$.current=Y_},[Y_]),b(()=>{b0.current=X},[X]),b(()=>{G$.current=P},[P]),b(()=>{h$.current=E},[E]),b(()=>{if(typeof window>"u"||typeof document>"u")return;let U=()=>$0(G9());U();let H=window.matchMedia?.("(prefers-color-scheme: dark)"),T=()=>U();if(H?.addEventListener)H.addEventListener("change",T);else if(H?.addListener)H.addListener(T);let I=typeof MutationObserver<"u"?new MutationObserver(()=>U()):null;if(I?.observe(document.documentElement,{attributes:!0,attributeFilter:["class","data-theme"]}),document.body)I?.observe(document.body,{attributes:!0,attributeFilter:["class","data-theme"]});return()=>{if(H?.removeEventListener)H.removeEventListener("change",T);else if(H?.removeListener)H.removeListener(T);I?.disconnect()}},[]),b(()=>{if(!P)return;let U=r_.current;if(!U)return;let H=requestAnimationFrame(()=>{try{U.focus(),U.select()}catch{}});return()=>cancelAnimationFrame(H)},[P]),b(()=>{if(!d)return;let U=(T)=>{let I=T?.target;if(!(I instanceof Element))return;if(e0.current?.contains(I))return;if(w.current?.contains(I))return;x_(!1)},H=(T)=>{if(T?.key==="Escape")x_(!1),w.current?.focus?.()};return document.addEventListener("mousedown",U),document.addEventListener("touchstart",U,{passive:!0}),document.addEventListener("keydown",H),()=>{document.removeEventListener("mousedown",U),document.removeEventListener("touchstart",U),document.removeEventListener("keydown",H)}},[d]);let L2=async(U)=>{N_(!0),x(null),A(null);try{let H=await x4(U,20000);x(H)}catch(H){x({error:H.message||"Failed to load preview"})}finally{N_(!1)}};b_.current=L2;let s0=async()=>{if(!F_.current)return;try{let U=await j1("",1,s.current),H=V9(U.root,Q_.current,s.current);if(H===D_.current){a(!1);return}if(D_.current=H,R_.current=U.root,!G0.current)G0.current=requestAnimationFrame(()=>{G0.current=0,W((T)=>y3(T,R_.current)),a(!1)})}catch(U){r(U.message||"Failed to load workspace"),a(!1)}},$$=async(U)=>{if(!U)return;if(n_.current.has(U))return;n_.current.add(U);try{let H=await j1(U,1,s.current);W((T)=>E3(T,U,H.root))}catch(H){r(H.message||"Failed to load workspace")}finally{n_.current.delete(U)}};c_.current=$$;let B0=M(()=>{let U=X;if(!U)return".";let H=p_.current?.get(U);if(H&&H.type==="dir")return H.path;if(U==="."||!U.includes("/"))return".";let T=U.split("/");return T.pop(),T.join("/")||"."},[X]),Z0=M((U)=>{let H=U?.closest?.(".workspace-row");if(!H)return null;let T=H.dataset.path,I=H.dataset.type;if(!T)return null;if(I==="dir")return T;if(T.includes("/")){let p=T.split("/");return p.pop(),p.join("/")||"."}return"."},[]),i$=M((U)=>{return Z0(U?.target||null)},[Z0]),a_=M((U)=>{i0.current=U,k_(U)},[]),v0=M(()=>{let U=P_.current;if(U?.timer)clearTimeout(U.timer);P_.current={path:null,timer:0}},[]),S0=M((U)=>{if(!U||U==="."){v0();return}let H=p_.current?.get(U);if(!H||H.type!=="dir"){v0();return}if(Q_.current?.has(U)){v0();return}if(P_.current?.path===U)return;v0();let T=setTimeout(()=>{P_.current={path:null,timer:0},c_.current?.(U),L((I)=>{let p=new Set(I);return p.add(U),p})},600);P_.current={path:U,timer:T}},[v0]),Q$=M((U,H)=>{if(_$.current={x:U,y:H},A_.current)return;A_.current=requestAnimationFrame(()=>{A_.current=0;let T=A0.current;if(!T)return;let I=_$.current;T.style.transform=`translate(${I.x+12}px, ${I.y+12}px)`})},[]),q$=M((U)=>{if(!U)return;let T=(p_.current?.get(U)?.name||U.split("/").pop()||U).trim();if(!T)return;U_({path:U,label:T})},[]),P$=M(()=>{if(U_(null),A_.current)cancelAnimationFrame(A_.current),A_.current=0;if(A0.current)A0.current.style.transform="translate(-9999px, -9999px)"},[]),x2=M((U)=>{if(!U)return".";let H=p_.current?.get(U);if(H&&H.type==="dir")return H.path;if(U==="."||!U.includes("/"))return".";let T=U.split("/");return T.pop(),T.join("/")||"."},[]),M$=M(()=>{C(null),u("")},[]),B2=M((U)=>{if(!U)return;let T=(p_.current?.get(U)?.name||U.split("/").pop()||U).trim();if(!T||U===".")return;C(U),u(T)},[]),w2=M(async()=>{let U=G$.current;if(!U)return;let H=(g||"").trim();if(!H){M$();return}let T=p_.current?.get(U),I=(T?.name||U.split("/").pop()||U).trim();if(H===I){M$();return}try{let t=(await C4(U,H))?.path||U,j_=U.includes("/")?U.split("/").slice(0,-1).join("/")||".":".";if(M$(),r(null),window.dispatchEvent(new CustomEvent("workspace-file-renamed",{detail:{oldPath:U,newPath:t,type:T?.type||"file"}})),T?.type==="dir")L((__)=>{let l=new Set;for(let B_ of __)if(B_===U)l.add(t);else if(B_.startsWith(`${U}/`))l.add(`${t}${B_.slice(U.length)}`);else l.add(B_);return l});if(F(t),T?.type==="dir")x(null),N_(!1),A(null);else b_.current?.(t);c_.current?.(j_)}catch(p){r(p?.message||"Failed to rename file")}},[M$,g]),W1=M(async(U)=>{let I=U||".";for(let p=0;p<50;p+=1){let j_=`untitled${p===0?"":`-${p}`}.md`;try{let l=(await f4(I,j_,""))?.path||(I==="."?j_:`${I}/${j_}`);if(I&&I!==".")L((B_)=>new Set([...B_,I]));F(l),r(null),c_.current?.(I),b_.current?.(l);return}catch(__){if(__?.status===409||__?.code==="file_exists")continue;r(__?.message||"Failed to create file");return}}r("Failed to create file (untitled name already in use).")},[]),l$=M((U)=>{if(U?.stopPropagation?.(),G_)return;let H=x2(b0.current);W1(H)},[G_,x2,W1]);b(()=>{if(typeof window>"u")return;let U=(H)=>{let T=H?.detail?.updates||[];if(!Array.isArray(T)||T.length===0)return;W((__)=>{let l=__;for(let B_ of T){if(!B_?.root)continue;if(!l||B_.path==="."||!B_.path)l=B_.root;else l=E3(l,B_.path,B_.root)}if(l)D_.current=V9(l,Q_.current,s.current);return a(!1),l});let I=b0.current;if(Boolean(I)&&T.some((__)=>{let l=__?.path||"";if(!l||l===".")return!0;return I===l||I.startsWith(`${l}/`)||l.startsWith(`${I}/`)}))E_.current.clear();if(!I||!h$.current)return;let t=p_.current?.get(I);if(t&&t.type==="dir")return;if(T.some((__)=>{let l=__?.path||"";if(!l||l===".")return!0;return I===l||I.startsWith(`${l}/`)}))b_.current?.(I)};return window.addEventListener("workspace-update",U),()=>window.removeEventListener("workspace-update",U)},[]),s_.current=s0;let j$=k(()=>{if(typeof window>"u")return;let U=window.matchMedia("(min-width: 1024px) and (orientation: landscape)"),H=M_.current??F_.current,T=document.visibilityState!=="hidden"&&(H||U.matches&&F_.current);Z1(T,s.current).catch(()=>{})}).current,X$=k(0),W$=k(()=>{if(X$.current)clearTimeout(X$.current);X$.current=setTimeout(()=>{X$.current=0,j$()},250)}).current;b(()=>{if(F_.current)s_.current?.();W$()},[$,j]),b(()=>{s_.current(),j$();let U=setInterval(()=>s_.current(),kZ),H=m2("previewHeight",null),T=Number.isFinite(H)?Math.min(Math.max(H,80),600):280;if(W0.current=T,x0.current)x0.current.style.setProperty("--preview-height",`${T}px`);let I=window.matchMedia("(min-width: 1024px) and (orientation: landscape)"),p=()=>W$();if(I.addEventListener)I.addEventListener("change",p);else if(I.addListener)I.addListener(p);return document.addEventListener("visibilitychange",p),()=>{if(clearInterval(U),G0.current)cancelAnimationFrame(G0.current),G0.current=0;if(I.removeEventListener)I.removeEventListener("change",p);else if(I.removeListener)I.removeListener(p);if(document.removeEventListener("visibilitychange",p),X$.current)clearTimeout(X$.current),X$.current=0;if(S_.current)clearTimeout(S_.current),S_.current=null;Z1(!1,s.current).catch(()=>{})}},[]);let y0=I_(()=>X9(G,Q,z_),[G,Q,z_]),o0=I_(()=>new Map(y0.map((U)=>[U.node.path,U.node])),[y0]),n$=I_(()=>K9(g_),[g_]);p_.current=o0;let U0=(X?p_.current.get(X):null)?.type==="dir";b(()=>{if(!X||!U0){V0(null),L0.current=null,f_.current=null;return}let U=X,H=`${z_?"hidden":"visible"}:${X}`,T=E_.current,I=T.get(H);if(I?.root){T.delete(H),T.set(H,I);let j_=Q9(I.root,Boolean(I.truncated),T_);if(j_)L0.current=j_,f_.current=X,V0({loading:!1,error:null,payload:j_});return}let p=L0.current,t=f_.current;V0({loading:!0,error:null,payload:t===X?p:null}),j1(X,IZ,z_).then((j_)=>{if(b0.current!==U)return;let __={root:j_?.root,truncated:Boolean(j_?.truncated)};T.delete(H),T.set(H,__);while(T.size>TZ){let B_=T.keys().next().value;if(!B_)break;T.delete(B_)}let l=Q9(__.root,__.truncated,T_);L0.current=l,f_.current=X,V0({loading:!1,error:null,payload:l})}).catch((j_)=>{if(b0.current!==U)return;V0({loading:!1,error:j_?.message||"Failed to load folder size chart",payload:t===X?p:null})})},[X,U0,z_,T_]);let O0=Boolean(E&&E.kind==="text"&&!U0&&(!E.size||E.size<=262144)),l0=O0?"Open in editor":E?.size>262144?"File too large to edit":"File is not editable",d$=Boolean(X&&X!=="."),R$=Boolean(X&&!U0),v$=Boolean(X&&!U0),u0=X&&U0?I1(X,z_):null,m0=M(()=>x_(!1),[]),t_=M(async(U)=>{m0();try{await U?.()}catch{}},[m0]);b(()=>{let U=C_.current;if(v_.current)v_.current.dispose(),v_.current=null;if(!U)return;if(U.innerHTML="",!X||U0||!E||E.error)return;let H={path:X,content:typeof E.text==="string"?E.text:void 0,mtime:E.mtime,size:E.size,preview:E,mode:"view"},T=N0.resolve(H)||N0.get("workspace-preview-default");if(!T)return;let I=T.mount(U,H);return v_.current=I,()=>{if(v_.current===I)I.dispose(),v_.current=null;U.innerHTML=""}},[X,U0,E]);let n0=(U)=>{let H=U?.target;if(H instanceof Element)return H;return H?.parentElement||null},L$=(U)=>{return Boolean(U?.closest?.(".workspace-node-icon, .workspace-label-text"))},B$=k((U)=>{let H=n0(U),T=H?.closest?.("[data-path]");if(!T)return;let I=T.dataset.path;if(!I||I===".")return;let p=Boolean(H?.closest?.("button"))||Boolean(H?.closest?.("a"))||Boolean(H?.closest?.("input")),t=Boolean(H?.closest?.(".workspace-caret"));if(p||t)return;if(G$.current===I)return;B2(I)}).current,r0=k((U)=>{if(Y0.current){Y0.current=!1;return}let H=n0(U),T=H?.closest?.("[data-path]");if(J0.current?.focus?.(),!T)return;let I=T.dataset.path,p=T.dataset.type,t=Boolean(H?.closest?.(".workspace-caret")),j_=Boolean(H?.closest?.("button"))||Boolean(H?.closest?.("a"))||Boolean(H?.closest?.("input")),__=b0.current===I,l=G$.current;if(l){if(l===I)return;M$()}let B_=p==="file"&&k$.current===I&&!t&&!j_;if(p==="dir"){if(k$.current=null,F(I),x(null),A(null),N_(!1),!Q_.current.has(I))c_.current?.(I);if(__&&!t)return;L((Z$)=>{let q0=new Set(Z$);if(q0.has(I))q0.delete(I);else q0.add(I);return q0})}else{k$.current=null,F(I);let M0=p_.current.get(I);if(M0)o_.current?.(M0.path,M0);if(!j_&&!t&&MZ(I))w_.current?.(I,h$.current);else b_.current?.(I)}}).current,k0=k(()=>{D_.current="",s_.current(),Array.from(Q_.current||[]).filter((H)=>H&&H!==".").forEach((H)=>c_.current?.(H))}).current,I$=k(()=>{k$.current=null,F(null),x(null),A(null),N_(!1)}).current,s$=k(()=>{f0((U)=>{let H=!U;if(typeof window<"u")e_("workspaceShowHidden",String(H));return s.current=H,Z1(!0,H).catch(()=>{}),D_.current="",s_.current?.(),Array.from(Q_.current||[]).filter((I)=>I&&I!==".").forEach((I)=>c_.current?.(I)),H})}).current,Y2=k((U)=>{if(n0(U)?.closest?.("[data-path]"))return;I$()}).current,T$=M(async(U)=>{if(!U)return;let H=U.split("/").pop()||U;if(!window.confirm(`Delete "${H}"? This cannot be undone.`))return;try{await S4(U);let I=U.includes("/")?U.split("/").slice(0,-1).join("/")||".":".";if(b0.current===U)I$();c_.current?.(I),r(null)}catch(I){x((p)=>({...p||{},error:I.message||"Failed to delete file"}))}},[I$]),u$=M((U)=>{let H=J0.current;if(!H||!U||typeof CSS>"u"||typeof CSS.escape!=="function")return;H.querySelector(`[data-path="${CSS.escape(U)}"]`)?.scrollIntoView?.({block:"nearest"})},[]),o$=M((U)=>{let H=y0;if(!H||H.length===0)return;let T=X?H.findIndex((I)=>I.node.path===X):-1;if(U.key==="ArrowDown"){U.preventDefault();let I=Math.min(T+1,H.length-1),p=H[I];if(!p)return;if(F(p.node.path),p.node.type!=="dir")o_.current?.(p.node.path,p.node),b_.current?.(p.node.path);else x(null),N_(!1),A(null);u$(p.node.path);return}if(U.key==="ArrowUp"){U.preventDefault();let I=T<=0?0:T-1,p=H[I];if(!p)return;if(F(p.node.path),p.node.type!=="dir")o_.current?.(p.node.path,p.node),b_.current?.(p.node.path);else x(null),N_(!1),A(null);u$(p.node.path);return}if(U.key==="ArrowRight"&&T>=0){let I=H[T];if(I?.node?.type==="dir"&&!Q.has(I.node.path))U.preventDefault(),c_.current?.(I.node.path),L((p)=>new Set([...p,I.node.path]));return}if(U.key==="ArrowLeft"&&T>=0){let I=H[T];if(I?.node?.type==="dir"&&Q.has(I.node.path))U.preventDefault(),L((p)=>{let t=new Set(p);return t.delete(I.node.path),t});return}if(U.key==="Enter"&&T>=0){U.preventDefault();let I=H[T];if(!I)return;let p=I.node.path;if(I.node.type==="dir"){if(!Q_.current.has(p))c_.current?.(p);L((j_)=>{let __=new Set(j_);if(__.has(p))__.delete(p);else __.add(p);return __}),x(null),A(null),N_(!1)}else o_.current?.(p,I.node),b_.current?.(p);return}if((U.key==="Delete"||U.key==="Backspace")&&T>=0){let I=H[T];if(!I||I.node.type==="dir")return;U.preventDefault(),T$(I.node.path);return}if(U.key==="Escape")U.preventDefault(),I$()},[I$,T$,Q,y0,u$,X]),P0=M((U)=>{let H=n0(U),T=H?.closest?.(".workspace-row");if(!T)return;let I=T.dataset.type,p=T.dataset.path;if(!p||p===".")return;if(G$.current===p)return;let t=U?.touches?.[0];if(!t)return;if(E0.current={path:L$(H)?p:null,dragging:!1,startX:t.clientX,startY:t.clientY},I!=="file")return;if(S_.current)clearTimeout(S_.current);S_.current=setTimeout(()=>{if(S_.current=null,E0.current?.dragging)return;T$(p)},600)},[T$]),x$=M(()=>{if(S_.current)clearTimeout(S_.current),S_.current=null;let U=E0.current;if(U?.dragging&&U.path){let H=i0.current||B0(),T=N2.current;if(typeof T==="function")T(U.path,H)}E0.current={path:null,dragging:!1,startX:0,startY:0},j0.current=0,$_(!1),V_(null),a_(null),v0(),P$()},[B0,P$,a_,v0]),r$=M((U)=>{let H=E0.current,T=U?.touches?.[0];if(!T||!H?.path){if(S_.current)clearTimeout(S_.current),S_.current=null;return}let I=Math.abs(T.clientX-H.startX),p=Math.abs(T.clientY-H.startY),t=I>8||p>8;if(t&&S_.current)clearTimeout(S_.current),S_.current=null;if(!H.dragging&&t)H.dragging=!0,$_(!0),V_("move"),q$(H.path);if(H.dragging){U.preventDefault(),Q$(T.clientX,T.clientY);let j_=document.elementFromPoint(T.clientX,T.clientY),__=Z0(j_)||B0();if(i0.current!==__)a_(__);S0(__)}},[Z0,B0,q$,Q$,a_,S0]),U2=k((U)=>{U.preventDefault();let H=x0.current;if(!H)return;let T=U.clientY,I=W0.current||280,p=U.currentTarget;p.classList.add("dragging"),document.body.style.cursor="row-resize",document.body.style.userSelect="none";let t=T,j_=(l)=>{t=l.clientY;let B_=H.clientHeight-80,M0=Math.min(Math.max(I-(l.clientY-T),80),B_);H.style.setProperty("--preview-height",`${M0}px`),W0.current=M0},__=()=>{let l=H.clientHeight-80,B_=Math.min(Math.max(I-(t-T),80),l);W0.current=B_,p.classList.remove("dragging"),document.body.style.cursor="",document.body.style.userSelect="",e_("previewHeight",String(Math.round(B_))),document.removeEventListener("mousemove",j_),document.removeEventListener("mouseup",__)};document.addEventListener("mousemove",j_),document.addEventListener("mouseup",__)}).current,z2=k((U)=>{U.preventDefault();let H=x0.current;if(!H)return;let T=U.touches[0];if(!T)return;let I=T.clientY,p=W0.current||280,t=U.currentTarget;t.classList.add("dragging"),document.body.style.userSelect="none";let j_=(l)=>{let B_=l.touches[0];if(!B_)return;l.preventDefault();let M0=H.clientHeight-80,Z$=Math.min(Math.max(p-(B_.clientY-I),80),M0);H.style.setProperty("--preview-height",`${Z$}px`),W0.current=Z$},__=()=>{t.classList.remove("dragging"),document.body.style.userSelect="",e_("previewHeight",String(Math.round(W0.current||p))),document.removeEventListener("touchmove",j_),document.removeEventListener("touchend",__),document.removeEventListener("touchcancel",__)};document.addEventListener("touchmove",j_,{passive:!1}),document.addEventListener("touchend",__),document.addEventListener("touchcancel",__)}).current,w$=async()=>{if(!X)return;try{let U=await w4(X);if(U.media_id)A(U.media_id)}catch(U){x((H)=>({...H||{},error:U.message||"Failed to attach"}))}},m$=async()=>{if(!X||U0)return;await T$(X)},g0=(U)=>{return Array.from(U?.dataTransfer?.types||[]).includes("Files")},a0=M((U)=>{if(!g0(U))return;if(U.preventDefault(),j0.current+=1,!y$.current)$_(!0);V_("upload");let H=i$(U)||B0();a_(H),S0(H)},[B0,i$,a_,S0]),a$=M((U)=>{if(!g0(U))return;if(U.preventDefault(),U.dataTransfer)U.dataTransfer.dropEffect="copy";if(!y$.current)$_(!0);if(V$.current!=="upload")V_("upload");let H=i$(U)||B0();if(i0.current!==H)a_(H);S0(H)},[B0,i$,a_,S0]),K2=M((U)=>{if(!g0(U))return;if(U.preventDefault(),j0.current=Math.max(0,j0.current-1),j0.current===0)$_(!1),V_(null),a_(null),v0()},[a_,v0]),f$=M(async(U,H=".")=>{let T=Array.from(U||[]);if(T.length===0)return;let I=H&&H!==""?H:".",p=I!=="."?I:"workspace root";d_(!0);try{let t=null;for(let j_ of T)try{t=await P1(j_,I)}catch(__){let l=__?.status,B_=__?.code;if(l===409||B_==="file_exists"){let M0=j_?.name||"file";if(!window.confirm(`"${M0}" already exists in ${p}. Overwrite?`))continue;t=await P1(j_,I,{overwrite:!0})}else throw __}if(t?.path)k$.current=t.path,F(t.path),b_.current?.(t.path);c_.current?.(I)}catch(t){r(t.message||"Failed to upload file")}finally{d_(!1)}},[]),V2=M(async(U,H)=>{if(!U)return;let T=p_.current?.get(U);if(!T)return;let I=H&&H!==""?H:".",p=U.includes("/")?U.split("/").slice(0,-1).join("/")||".":".";if(I===p)return;try{let j_=(await b4(U,I))?.path||U;if(T.type==="dir")L((__)=>{let l=new Set;for(let B_ of __)if(B_===U)l.add(j_);else if(B_.startsWith(`${U}/`))l.add(`${j_}${B_.slice(U.length)}`);else l.add(B_);return l});if(F(j_),T.type==="dir")x(null),N_(!1),A(null);else b_.current?.(j_);c_.current?.(p),c_.current?.(I)}catch(t){r(t?.message||"Failed to move entry")}},[]);N2.current=V2;let F0=M(async(U)=>{if(!g0(U))return;U.preventDefault(),j0.current=0,$_(!1),V_(null),k_(null),v0();let H=Array.from(U?.dataTransfer?.files||[]);if(H.length===0)return;let T=i0.current||i$(U)||B0();await f$(H,T)},[B0,i$,f$]),t$=M((U)=>{if(U?.stopPropagation?.(),G_)return;let H=U?.currentTarget?.dataset?.uploadTarget||".";X0.current=H,Q0.current?.click()},[G_]),g$=M(()=>{if(G_)return;let U=b0.current,H=U?p_.current?.get(U):null;X0.current=H?.type==="dir"?H.path:".",Q0.current?.click()},[G_]),O2=M(()=>{t_(()=>l$(null))},[t_,l$]),U$=M(()=>{t_(()=>g$())},[t_,g$]),G2=M(()=>{t_(()=>k0())},[t_,k0]),F2=M(()=>{t_(()=>s$())},[t_,s$]),H2=M(()=>{if(!X||!O0)return;t_(()=>w_.current?.(X,E))},[t_,X,O0,E]),c2=M(()=>{if(!X||X===".")return;t_(()=>B2(X))},[t_,X,B2]),D2=M(()=>{if(!X||U0)return;t_(()=>m$())},[t_,X,U0,m$]),L_=M(()=>{if(!X||U0)return;t_(()=>w$())},[t_,X,U0,w$]),f2=M(()=>{if(!u0)return;if(m0(),typeof window<"u")window.open(u0,"_blank","noopener")},[m0,u0]),h2=M(()=>{m0(),N?.()},[m0,N]),i2=M(()=>{m0(),Y?.()},[m0,Y]),C$=M((U)=>{if(!U||U.button!==0)return;let H=U.currentTarget;if(!H||!H.dataset)return;let T=H.dataset.path;if(!T||T===".")return;if(G$.current===T)return;let I=n0(U);if(I?.closest?.("button, a, input, .workspace-caret"))return;if(!L$(I))return;U.preventDefault(),w0.current={path:T,dragging:!1,startX:U.clientX,startY:U.clientY};let p=(j_)=>{let __=w0.current;if(!__?.path)return;let l=Math.abs(j_.clientX-__.startX),B_=Math.abs(j_.clientY-__.startY),M0=l>4||B_>4;if(!__.dragging&&M0)__.dragging=!0,Y0.current=!0,$_(!0),V_("move"),q$(__.path),Q$(j_.clientX,j_.clientY),document.body.style.userSelect="none",document.body.style.cursor="grabbing";if(__.dragging){j_.preventDefault(),Q$(j_.clientX,j_.clientY);let Z$=document.elementFromPoint(j_.clientX,j_.clientY),q0=Z0(Z$)||B0();if(i0.current!==q0)a_(q0);S0(q0)}},t=()=>{document.removeEventListener("mousemove",p),document.removeEventListener("mouseup",t);let j_=w0.current;if(j_?.dragging&&j_.path){let __=i0.current||B0(),l=N2.current;if(typeof l==="function")l(j_.path,__)}w0.current={path:null,dragging:!1,startX:0,startY:0},j0.current=0,$_(!1),V_(null),a_(null),v0(),P$(),document.body.style.userSelect="",document.body.style.cursor="",setTimeout(()=>{Y0.current=!1},0)};document.addEventListener("mousemove",p),document.addEventListener("mouseup",t)},[Z0,B0,q$,Q$,P$,a_,S0,v0]),l2=M(async(U)=>{let H=Array.from(U?.target?.files||[]);if(H.length===0)return;let T=X0.current||".";if(await f$(H,T),X0.current=".",U?.target)U.target.value=""},[f$]);return B`
        <aside
                    ...
                    <div class="workspace-menu-wrap">
                        ...
                        ${d&&B`
                            <div class="workspace-menu-dropdown" ref=${e0} role="menu" aria-label="Workspace options">
                                ...
                                `}
                                ${v$&&B`
                                    <button class="workspace-menu-item" role="menuitem" onClick=${L_}>Download selected file</button>
                                `}
                                ${u0&&B`
            ...
            <div class="workspace-tree" onClick=${Y2}>
                ...
                ${G&&B`
                    <div
                        ...
                        onTouchCancel=${x$}
                    >
                        ${y0.map(({node:U,depth:H})=>{let T=U.type==="dir",I=U.path===X,p=U.path===P,t=T&&Q.has(U.path),j_=W_&&U.path===W_,__=Array.isArray(U.children)&&U.children.length>0?U.children.length:Number(U.child_count)||0;return B`
                                <div
                                    key=${U.path}
    ...
    `}var fZ=/\.(docx?|xlsx?|pptx?|odt|ods|odp|rtf)$/i,CZ=/\.(csv|tsv)$/i,bZ=/\.pdf$/i,SZ=/\.(png|jpe?g|gif|webp|bmp|ico|svg)$/i,J9=/\.drawio(\.xml|\.svg|\.png)?$/i;function E9({tabs:_,activeId:$,onActivate:j,onClose:Z,onCloseOthers:N,onCloseAll:Y,onTogglePin:z,onTogglePreview:G,previewTabs:W,onToggleDock:Q,dockVisible:L,onToggleZen:X,zenMode:F}){let[P,C]=v(null),g=k(null);b(()=>{if(!P)return;let A=(S)=>{if(S.type==="keydown"&&S.key!=="Escape")return;C(null)};return document.addEventListener("click",A),document.addEventListener("keydown",A),()=>{document.removeEventListener("click",A),document.removeEventListener("keydown",A)}},[P]),b(()=>{let A=(S)=>{if(S.ctrlKey&&S.key==="Tab"){if(S.preventDefault(),!_.length)return;let a=_.findIndex((Z_)=>Z_.id===$);if(S.shiftKey){let Z_=_[(a-1+_.length)%_.length];j?.(Z_.id)}else{let Z_=_[(a+1)%_.length];j?.(Z_.id)}return}if((S.ctrlKey||S.metaKey)&&S.key==="w"){let a=document.querySelector(".editor-pane");if(a&&a.contains(document.activeElement)){if(S.preventDefault(),$)Z?.($)}}};return document.addEventListener("keydown",A),()=>document.removeEventListener("keydown",A)},[_,$,j,Z]);let u=M((A,S)=>{if(A.button===1){A.preventDefault(),Z?.(S);return}if(A.button===0)j?.(S)},[j,Z]),E=M((A,S)=>{A.preventDefault(),C({id:S,x:A.clientX,y:A.clientY})},[]),x=M((A)=>{A.preventDefault(),A.stopPropagation()},[]),m=M((A,S)=>{A.preventDefault(),A.stopPropagation(),Z?.(S)},[Z]);if(b(()=>{if(!$||!g.current)return;let A=g.current.querySelector(".tab-item.active");if(A)A.scrollIntoView({block:"nearest",inline:"nearest",behavior:"smooth"})},[$]),!_.length)return null;return B`
            ...
            <div class="tab-context-menu" style=${{left:P.x+"px",top:P.y+"px"}}>
                ...
                ${G&&/\.(md|mdx|markdown)$/i.test(P.id)&&B`
                    ...
                    <button onClick=${()=>{G(P.id),C(null)}}>
                        ${W?.has(P.id)?"Hide Preview":"Preview"}
                    </button>
                `}
                ${fZ.test(P.id)&&B`
                    <hr />
        ...
        `}
    `}var RZ=400,P3=60,A9=220,M3="mdPreviewHeight";function vZ(){try{let _=localStorage.getItem(M3),$=_?Number(_):NaN;return Number.isFinite($)&&$>=P3?$:A9}catch{return A9}}function y9({getContent:_,path:$,onClose:j}){let[Z,N]=v(""),[Y,z]=v(vZ),G=k(null),W=k(null),Q=k(""),L=k(_);return L.current=_,b(()=>{let P=()=>{let g=L.current?.()||"";if(g===Q.current)return;Q.current=g;try{let u=K$(g,null,{sanitize:!1});N(u)}catch{N('<p style="color:var(--text-secondary)">Preview unavailable</p>')}};P();let C=setInterval(P,RZ);return()=>clearInterval(C)},[]),b(()=>{if(G.current&&Z)j2(G.current).catch(()=>{})},[Z]),B`
        <div
            class="md-preview-splitter"
            onMouseDown=${(P)=>{P.preventDefault();let C=P.clientY,g=W.current?.offsetHeight||Y,u=W.current?.parentElement,E=u?u.offsetHeight*0.7:500,x=P.currentTarget;x.classList.add("dragging"),document.body.style.cursor="row-resize",document.body.style.userSelect="none";let m=(S)=>{let a=Math.min(Math.max(g-(S.clientY-C),P3),E);z(a)},A=()=>{x.classList.remove("dragging"),document.body.style.cursor="",document.body.style.userSelect="";try{localStorage.setItem(M3,String(Math.round(W.current?.offsetHeight||Y)))}catch{}document.removeEventListener("mousemove",m),document.removeEventListener("mouseup",A)};document.addEventListener("mousemove",m),document.addEventListener("mouseup",A)}}
            onTouchStart=${(P)=>{P.preventDefault();let C=P.touches[0];if(!C)return;let g=C.clientY,u=W.current?.offsetHeight||Y,E=W.current?.parentElement,x=E?E.offsetHeight*0.7:500,m=P.currentTarget;m.classList.add("dragging"),document.body.style.userSelect="none";let A=(a)=>{let Z_=a.touches[0];if(!Z_)return;a.preventDefault();let N_=Math.min(Math.max(u-(Z_.clientY-g),P3),x);z(N_)},S=()=>{m.classList.remove("dragging"),document.body.style.userSelect="";try{localStorage.setItem(M3,String(Math.round(W.current?.offsetHeight||Y)))}catch{}document.removeEventListener("touchmove",A),document.removeEventListener("touchend",S),document.removeEventListener("touchcancel",S)};document.addEventListener("touchmove",A,{passive:!1}),document.addEventListener("touchend",S),document.addEventListener("touchcancel",S)}}
        ></div>
    ...
    `}function k9({handleSseEvent:_,handleConnectionStatusChange:$,loadPosts:j,onWake:Z,chatJid:N}){let Y=k(_);Y.current=_;let z=k($);z.current=$;let G=k(j);G.current=j;let W=k(Z);W.current=Z,b(()=>{G.current();let Q=new T1((X,F)=>Y.current(X,F),(X)=>z.current(X),{chatJid:N});Q.connect();let L=()=>{Q.reconnectIfNeeded();let X=typeof document<"u"?document:null;if(!X||X.visibilityState==="visible")W.current?.()};return window.addEventListener("focus",L),document.addEventListener("visibilitychange",L),()=>{window.removeEventListener("focus",L),document.removeEventListener("visibilitychange",L),Q.disconnect()}},[N])}function P9(){let[_,$]=v(!1),[j,Z]=v("default"),N=k(!1);b(()=>{let W=u2("notificationsEnabled",!1);if(N.current=W,$(W),typeof Notification<"u")Z(Notification.permission)},[]),b(()=>{N.current=_},[_]);let Y=M(()=>{if(typeof Notification>"u")return Promise.resolve("denied");try{let W=Notification.requestPermission();if(W&&typeof W.then==="function")return W;return Promise.resolve(W)}catch{return Promise.resolve("default")}},[]),z=M(async()=>{if(typeof window>"u"||typeof Notification>"u")return;if(!window.isSecureContext){alert("Notifications require a secure context (HTTPS or installed app).");return}if(Notification.permission==="denied"){Z("denied"),alert("Browser notifications are blocked. Enable them in your browser settings.");return}if(Notification.permission==="default"){let Q=await Y();if(Z(Q||"default"),Q!=="granted"){N.current=!1,$(!1),e_("notificationsEnabled","false");return}}let W=!N.current;N.current=W,$(W),e_("notificationsEnabled",String(W))},[Y]),G=M((W,Q)=>{if(!N.current)return!1;if(typeof Notification>"u")return!1;if(Notification.permission!=="granted")return!1;try{let L=new Notification(W,{body:Q});return L.onclick=()=>{try{window.focus()}catch{}},!0}catch{return!1}},[]);return{notificationsEnabled:_,notificationPermission:j,toggleNotifications:z,notify:G}}var X1=(_)=>{let $=new Set;return(_||[]).filter((j)=>{if(!j||$.has(j.id))return!1;return $.add(j.id),!0})};function M9({preserveTimelineScroll:_,preserveTimelineScrollTop:$,chatJid:j=null}){let[Z,N]=v(null),[Y,z]=v(!1),G=k(!1),W=k(null),Q=k(!1),L=k(null),X=k(null),F=k(0);b(()=>{G.current=Y},[Y]),b(()=>{X.current=Z},[Z]),b(()=>{F.current+=1,L.current=null,Q.current=!1,G.current=!1,z(!1)},[j]);let P=M(async(u=null)=>{let E=F.current;try{if(u){let x=await U4(u,50,0,j);if(E!==F.current)return;N(x.posts),z(!1)}else{let x=await A2(10,null,j);if(E!==F.current)return;N(x.posts),z(x.has_more)}}catch(x){if(E!==F.current)return;console.error("Failed to load posts:",x)}},[j]),C=M(async()=>{let u=F.current;try{let E=await A2(10,null,j);if(u!==F.current)return;N((x)=>{if(!x||x.length===0)return E.posts;return X1([...E.posts,...x])}),z((x)=>x||E.has_more)}catch(E){if(u!==F.current)return;console.error("Failed to refresh timeline:",E)}},[j]),g=M(async(u={})=>{let E=F.current,x=X.current;if(!x||x.length===0)return;if(Q.current)return;let{preserveScroll:m=!0,preserveMode:A="top",allowRepeat:S=!1}=u,a=(e)=>{if(!m){e();return}if(A==="top")$(e);else _(e)},N_=x.slice().sort((e,r)=>e.id-r.id)[0]?.id;if(!Number.isFinite(N_))return;if(!S&&L.current===N_)return;Q.current=!0,L.current=N_;try{let e=await A2(10,N_,j);if(E!==F.current)return;if(e.posts.length>0)a(()=>{N((r)=>X1([...e.posts,...r||[]])),z(e.has_more)});else z(!1)}catch(e){if(E!==F.current)return;console.error("Failed to load more posts:",e)}finally{if(E===F.current)Q.current=!1}},[j,_,$]);return b(()=>{W.current=g},[g]),{posts:Z,setPosts:N,hasMore:Y,setHasMore:z,hasMoreRef:G,loadPosts:P,refreshTimeline:C,loadMore:g,loadMoreRef:W,loadingMoreRef:Q,lastBeforeIdRef:L}}function I9(){let[_,$]=v(null),[j,Z]=v({text:"",totalLines:0}),[N,Y]=v(""),[z,G]=v({text:"",totalLines:0}),[W,Q]=v(null),[L,X]=v(null),[F,P]=v(null),C=k(null),g=k(0),u=k(!1),E=k(""),x=k(""),m=k(null),A=k(null),S=k(null),a=k(null),Z_=k(!1),N_=k(!1);return{agentStatus:_,setAgentStatus:$,agentDraft:j,setAgentDraft:Z,agentPlan:N,setAgentPlan:Y,agentThought:z,setAgentThought:G,pendingRequest:W,setPendingRequest:Q,currentTurnId:L,setCurrentTurnId:X,steerQueuedTurnId:F,setSteerQueuedTurnId:P,lastAgentEventRef:C,lastSilenceNoticeRef:g,isAgentRunningRef:u,draftBufferRef:E,thoughtBufferRef:x,pendingRequestRef:m,stalledPostIdRef:A,currentTurnIdRef:S,steerQueuedTurnIdRef:a,thoughtExpandedRef:Z_,draftExpandedRef:N_}}function T9({appShellRef:_,sidebarWidthRef:$,editorWidthRef:j,dockHeightRef:Z}){let N=k((L)=>{L.preventDefault();let X=_.current;if(!X)return;let F=L.clientX,P=$.current||280,C=L.currentTarget;C.classList.add("dragging"),X.classList.add("sidebar-resizing"),document.body.style.cursor="col-resize",document.body.style.userSelect="none";let g=F,u=(x)=>{g=x.clientX;let m=Math.min(Math.max(P+(x.clientX-F),160),600);X.style.setProperty("--sidebar-width",`${m}px`),$.current=m},E=()=>{let x=Math.min(Math.max(P+(g-F),160),600);$.current=x,C.classList.remove("dragging"),X.classList.remove("sidebar-resizing"),document.body.style.cursor="",document.body.style.userSelect="",e_("sidebarWidth",String(Math.round(x))),document.removeEventListener("mousemove",u),document.removeEventListener("mouseup",E)};document.addEventListener("mousemove",u),document.addEventListener("mouseup",E)}).current,Y=k((L)=>{L.preventDefault();let X=_.current;if(!X)return;let F=L.touches[0];if(!F)return;let P=F.clientX,C=$.current||280,g=L.currentTarget;g.classList.add("dragging"),X.classList.add("sidebar-resizing"),document.body.style.userSelect="none";let u=(x)=>{let m=x.touches[0];if(!m)return;x.preventDefault();let A=Math.min(Math.max(C+(m.clientX-P),160),600);X.style.setProperty("--sidebar-width",`${A}px`),$.current=A},E=()=>{g.classList.remove("dragging"),X.classList.remove("sidebar-resizing"),document.body.style.userSelect="",e_("sidebarWidth",String(Math.round($.current||C))),document.removeEventListener("touchmove",u),document.removeEventListener("touchend",E),document.removeEventListener("touchcancel",E)};document.addEventListener("touchmove",u,{passive:!1}),document.addEventListener("touchend",E),document.addEventListener("touchcancel",E)}).current,z=k((L)=>{L.preventDefault();let X=_.current;if(!X)return;let F=L.clientX,P=j.current||$.current||280,C=L.currentTarget;C.classList.add("dragging"),document.body.style.cursor="col-resize",document.body.style.userSelect="none";let g=F,u=(x)=>{g=x.clientX;let m=Math.min(Math.max(P+(x.clientX-F),200),800);X.style.setProperty("--editor-width",`${m}px`),j.current=m},E=()=>{let x=Math.min(Math.max(P+(g-F),200),800);j.current=x,C.classList.remove("dragging"),document.body.style.cursor="",document.body.style.userSelect="",e_("editorWidth",String(Math.round(x))),document.removeEventListener("mousemove",u),document.removeEventListener("mouseup",E)};document.addEventListener("mousemove",u),document.addEventListener("mouseup",E)}).current,G=k((L)=>{L.preventDefault();let X=_.current;if(!X)return;let F=L.touches[0];if(!F)return;let P=F.clientX,C=j.current||$.current||280,g=L.currentTarget;g.classList.add("dragging"),document.body.style.userSelect="none";let u=(x)=>{let m=x.touches[0];if(!m)return;x.preventDefault();let A=Math.min(Math.max(C+(m.clientX-P),200),800);X.style.setProperty("--editor-width",`${A}px`),j.current=A},E=()=>{g.classList.remove("dragging"),document.body.style.userSelect="",e_("editorWidth",String(Math.round(j.current||C))),document.removeEventListener("touchmove",u),document.removeEventListener("touchend",E),document.removeEventListener("touchcancel",E)};document.addEventListener("touchmove",u,{passive:!1}),document.addEventListener("touchend",E),document.addEventListener("touchcancel",E)}).current,W=k((L)=>{L.preventDefault();let X=_.current;if(!X)return;let F=L.clientY,P=Z?.current||200,C=L.currentTarget;C.classList.add("dragging"),document.body.style.cursor="row-resize",document.body.style.userSelect="none";let g=F,u=(x)=>{g=x.clientY;let m=Math.min(Math.max(P-(x.clientY-F),100),window.innerHeight*0.5);if(X.style.setProperty("--dock-height",`${m}px`),Z)Z.current=m;window.dispatchEvent(new CustomEvent("dock-resize"))},E=()=>{let x=Math.min(Math.max(P-(g-F),100),window.innerHeight*0.5);if(Z)Z.current=x;C.classList.remove("dragging"),document.body.style.cursor="",document.body.style.userSelect="",e_("dockHeight",String(Math.round(x))),window.dispatchEvent(new CustomEvent("dock-resize")),document.removeEventListener("mousemove",u),document.removeEventListener("mouseup",E)};document.addEventListener("mousemove",u),document.addEventListener("mouseup",E)}).current,Q=k((L)=>{L.preventDefault();let X=_.current;if(!X)return;let F=L.touches[0];if(!F)return;let P=F.clientY,C=Z?.current||200,g=L.currentTarget;g.classList.add("dragging"),document.body.style.userSelect="none";let u=(x)=>{let m=x.touches[0];if(!m)return;x.preventDefault();let A=Math.min(Math.max(C-(m.clientY-P),100),window.innerHeight*0.5);if(X.style.setProperty("--dock-height",`${A}px`),Z)Z.current=A;window.dispatchEvent(new CustomEvent("dock-resize"))},E=()=>{g.classList.remove("dragging"),document.body.style.userSelect="",e_("dockHeight",String(Math.round(Z?.current||C))),window.dispatchEvent(new CustomEvent("dock-resize")),document.removeEventListener("touchmove",u),document.removeEventListener("touchend",E),document.removeEventListener("touchcancel",E)};document.addEventListener("touchmove",u,{passive:!1}),document.addEventListener("touchend",E),document.addEventListener("touchcancel",E)}).current;return{handleSplitterMouseDown:N,handleSplitterTouchStart:Y,handleEditorSplitterMouseDown:z,handleEditorSplitterTouchStart:G,handleDockSplitterMouseDown:W,handleDockSplitterTouchStart:Q}}function x9({onTabClosed:_}={}){let $=k(_);$.current=_;let[j,Z]=v(()=>_0.getTabs()),[N,Y]=v(()=>_0.getActiveId()),[z,G]=v(()=>_0.getTabs().length>0);b(()=>{return _0.onChange((A,S)=>{Z(A),Y(S),G(A.length>0)})},[]);let[W,Q]=v(()=>new Set),L=M((A)=>{Q((S)=>{let a=new Set(S);if(a.has(A))a.delete(A);else a.add(A);return a})},[]),X=M((A)=>{Q((S)=>{if(!S.has(A))return S;let a=new Set(S);return a.delete(A),a})},[]),F=M((A,S={})=>{if(!A)return;let a={path:A,mode:"edit"};try{if(!N0.resolve(a)){if(!N0.get("editor")){console.warn(`[openEditor] No pane handler for: ${A}`);return}}}catch(N_){console.warn(`[openEditor] paneRegistry.resolve() error for "${A}":`,N_)}let Z_=typeof S?.label==="string"&&S.label.trim()?S.label.trim():void 0;_0.open(A,Z_)},[]),P=M(()=>{let A=_0.getActiveId();if(A){let S=_0.get(A);if(S?.dirty){if(!window.confirm(`"${S.label}" has unsaved changes. Close anyway?`))return}_0.close(A),X(A),$.current?.(A)}},[X]),C=M((A)=>{let S=_0.get(A);if(S?.dirty){if(!window.confirm(`"${S.label}" has unsaved changes. Close anyway?`))return}_0.close(A),X(A),$.current?.(A)},[X]),g=M((A)=>{_0.activate(A)},[]),u=M((A)=>{let S=_0.getTabs().filter((N_)=>N_.id!==A&&!N_.pinned),a=S.filter((N_)=>N_.dirty).length;if(a>0){if(!window.confirm(`${a} unsaved tab${a>1?"s":""} will be closed. Continue?`))return}let Z_=S.map((N_)=>N_.id);_0.closeOthers(A),Z_.forEach((N_)=>{X(N_),$.current?.(N_)})},[X]),E=M(()=>{let A=_0.getTabs().filter((Z_)=>!Z_.pinned),S=A.filter((Z_)=>Z_.dirty).length;if(S>0){if(!window.confirm(`${S} unsaved tab${S>1?"s":""} will be closed. Continue?`))return}let a=A.map((Z_)=>Z_.id);_0.closeAll(),a.forEach((Z_)=>{X(Z_),$.current?.(Z_)})},[X]),x=M((A)=>{_0.togglePin(A)},[]),m=M(()=>{let A=_0.getActiveId();if(A)window.dispatchEvent(new CustomEvent("workspace-reveal-path",{detail:{path:A}}))},[]);return b(()=>{let A=(S)=>{let{oldPath:a,newPath:Z_,type:N_}=S.detail||{};if(!a||!Z_)return;if(N_==="dir"){for(let e of _0.getTabs())if(e.path===a||e.path.startsWith(`${a}/`)){let r=`${Z_}${e.path.slice(a.length)}`;_0.rename(e.id,r)}}else _0.rename(a,Z_)};return window.addEventListener("workspace-file-renamed",A),()=>window.removeEventListener("workspace-file-renamed",A)},[]),b(()=>{let A=(S)=>{if(_0.hasUnsaved())S.preventDefault(),S.returnValue=""};return window.addEventListener("beforeunload",A),()=>window.removeEventListener("beforeunload",A)},[]),{editorOpen:z,tabStripTabs:j,tabStripActiveId:N,previewTabs:W,openEditor:F,closeEditor:P,handleTabClose:C,handleTabActivate:g,handleTabCloseOthers:u,handleTabCloseAll:E,handleTabTogglePin:x,handleTabTogglePreview:L,revealInExplorer:m}}function I3(_,$){try{if(typeof window>"u")return $;let j=window.__PICLAW_SILENCE||{},Z=`__PICLAW_SILENCE_${_.toUpperCase()}_MS`,N=j[_]??window[Z],Y=Number(N);return Number.isFinite(Y)?Y:$}catch{return $}}var T3=I3("warning",30000),w9=I3("finalize",120000),x3=I3("refresh",30000),f9=30000;function C9(_){let $={};return(_?.agents||[]).forEach((j)=>{$[j.id]=j}),$}function b9(){if(/iPad|iPhone/.test(navigator.userAgent))return!0;return navigator.platform==="MacIntel"&&navigator.maxTouchPoints>1}function S9(_=30000){let[,$]=v(0);b(()=>{let j=setInterval(()=>$((Z)=>Z+1),_);return()=>clearInterval(j)},[_])}function w3(_,$=160){let j=String(_||"").replace(/\r\n/g,`
...
This archives the branch agent and removes it from the branch picker. Chat history is preserved.`))return;try{await aZ(D),await Promise.allSettled([p0(),R0()]);let i=y?.root_chat_jid||"web:default";L_("Branch pruned",`${n} has been archived.`,"info",3000);let c=T2(window.location.href,i,{chatOnly:Z});$?.(c)}catch(i){let c=i instanceof Error?i.message:String(i||"Could not prune branch.");L_("Could not prune branch",c||"Could not prune branch.","warning",5000)}},[J0,Z,w,Q0,j,$,p0,R0,L_]),H5=M(async(V)=>{let K=typeof V==="string"?V.trim():"";if(!K||typeof Z5!=="function")return;try{let O=await Z5(K);await Promise.allSettled([p0(),R0()]);let D=O?.branch,y=typeof D?.chat_jid==="string"&&D.chat_jid.trim()?D.chat_jid.trim():K,R=typeof D?.agent_name==="string"&&D.agent_name.trim()?`@${D.agent_name.trim()}`:y;L_("Branch restored",`Restored ${R}.`,"info",3200);let n=T2(window.location.href,y,{chatOnly:Z});$?.(n)}catch(O){let D=O instanceof Error?O.message:String(O||"Could not restore branch.");L_("Could not restore branch",D||"Could not restore branch.","warning",5000)}},[Z,$,p0,R0,L_]);b(()=>{if(!N||typeof window>"u")return;let V=!1;return(async()=>{try{j0({status:"running",message:"Preparing a new chat branch…"});let K=await _1(Y);if(V)return;let O=K?.branch,D=typeof O?.chat_jid==="string"&&O.chat_jid.trim()?O.chat_jid.trim():null;if(!D)throw Error("Branch fork did not return a chat id.");let y=T2(window.location.href,D,{chatOnly:!0});$?.(y,{replace:!0})}catch(K){if(V)return;j0({status:"error",message:s1(K)})}})(),()=>{V=!0}},[N,Y,$]);let D5=M((V)=>{if(!V||typeof V!=="object")return;let K=c0(V);if(K)v_.current.delete(K);C_({...V,openedAt:new Date().toISOString()})},[]),a2=M(()=>{C_((V)=>{let K=c0(V);if(V?.source==="live"&&K)v_.current.add(K);return null})},[]),J5=M((V,K)=>{let O=typeof V?.kind==="string"?V.kind:"",D=c0(K);if(!O||!D)return;if(O==="widget.close"){a2();return}if(O==="widget.submit"){let y=Q6(V?.payload),R=q6(V?.payload),n=new Date().toISOString();if(C_((o)=>{let i=c0(o);if(!o||i!==D)return o;return{...o,runtimeState:{...o.runtimeState||{},lastEventKind:O,lastEventPayload:V?.payload||null,lastSubmitAt:n,lastHostUpdate:{type:"submit_pending",submittedAt:n,preview:y||null}}}}),!y){if(L_("Widget submission received","The widget submitted data without a message payload yet.","info",3500),R)a2();return}(async()=>{try{let o=await y2("default",y,null,[],n2?"queue":null,j);if(s2(o),C_((i)=>{let c=c0(i);if(!i||c!==D)return i;return{...i,runtimeState:{...i.runtimeState||{},lastHostUpdate:{type:o?.queued==="followup"?"submit_queued":"submit_sent",submittedAt:n,preview:y,queued:o?.queued||null}}}}),L_(o?.queued==="followup"?"Widget submission queued":"Widget submission sent",o?.queued==="followup"?"The widget message was queued because the agent is busy.":"The widget message was sent to the chat.","info",3500),R)a2()}catch(o){C_((i)=>{let c=c0(i);if(!i||c!==D)return i;return{...i,runtimeState:{...i.runtimeState||{},lastHostUpdate:{type:"submit_failed",submittedAt:n,preview:y,error:o?.message||"Could not send the widget message."}}}}),L_("Widget submission failed",o?.message||"Could not send the widget message.","warning",5000)}})();return}if(O==="widget.ready"||O==="widget.request_refresh"){let y=new Date().toISOString(),R=Boolean(V?.payload?.buildDashboard||V?.payload?.dashboardKind==="internal-state"),n=Number(K?.runtimeState?.refreshCount||0)+1;if(C_((o)=>{let i=c0(o);if(!o||i!==D)return o;return{...o,runtimeState:{...o.runtimeState||{},lastEventKind:O,lastEventPayload:V?.payload||null,...O==="widget.ready"?{readyAt:y,lastHostUpdate:{type:"ready_ack",at:y}}:{},...O==="widget.request_refresh"?{lastRefreshRequestAt:y,refreshCount:n,lastHostUpdate:{type:R?"refresh_building":"refresh_ack",at:y,count:n,echo:V?.payload||null}}:{}}}}),O==="widget.request_refresh")if(R)(async()=>{try{let o=await p3(V?.payload||null);C_((i)=>{let c=c0(i);if(!i||c!==D)return i;return{...i,runtimeState:{...i.runtimeState||{},dashboard:o,lastHostUpdate:{type:"refresh_dashboard",at:new Date().toISOString(),count:n,echo:V?.payload||null}}}}),L_("Dashboard built","Live dashboard state pushed into the widget.","info",3000)}catch(o){C_((i)=>{let c=c0(i);if(!i||c!==D)return i;return{...i,runtimeState:{...i.runtimeState||{},lastHostUpdate:{type:"refresh_failed",at:new Date().toISOString(),count:n,error:o?.message||"Could not build dashboard."}}}}),L_("Dashboard build failed",o?.message||"Could not build dashboard.","warning",5000)}})();else L_("Widget refresh requested","The widget received a host acknowledgement update.","info",3000)}},[p3,j,a2,s2,n2,L_]);b(()=>{v_.current.clear(),C_(null)},[j]);let E5=M(async()=>{if(typeof window>"u")return;try{let K=(await _1(j))?.branch,O=typeof K?.chat_jid==="string"&&K.chat_jid.trim()?K.chat_jid.trim():null;if(!O)throw Error("Branch fork did not return a chat id.");await Promise.allSettled([p0(),R0()]);let D=K?.agent_name?`@${K.agent_name}`:O;L_("New branch created",`Switched to ${D}.`,"info",2500);let y=T2(window.location.href,O,{chatOnly:Z});$?.(y)}catch(V){L_("Could not create branch",s1(V),"warning",5000)}},[Z,j,$,p0,R0,L_]),A5=M(async()=>{if(typeof window>"u"||W)return;let V=v9(j);if(!V){L_("Could not open branch window","Opening branch windows is unavailable in standalone webapp mode.","warning",5000);return}if(V.mode==="tab"){let O=c9(window.location.href,j,{chatOnly:!0});if(!window.open(O,V.target))L_("Could not open branch window","The browser blocked opening a new tab or window.","warning",5000);return}let K=u9(V);if(!K){L_("Could not open branch window","The browser blocked opening a new tab or window.","warning",5000);return}m9(K,{title:"Opening branch…",message:"Preparing a new chat branch. This should only take a moment."});try{let D=(await _1(j))?.branch,y=typeof D?.chat_jid==="string"&&D.chat_jid.trim()?D.chat_jid.trim():null;if(!y)throw Error("Branch fork did not return a chat id.");try{let n=await D4();r_(Array.isArray(n?.chats)?n.chats:[])}catch{}try{let n=await r1(s);X0(Array.isArray(n?.chats)?n.chats:[])}catch{}let R=T2(window.location.href,y,{chatOnly:!0});g9(K,R)}catch(O){p9(K),L_("Could not open branch window",s1(O),"error",5000)}},[j,s,W,L_]);b(()=>{if(!Z0)return;if(typeof window>"u")return;let V=r$.current;if(!V)return;if(!z2.current){let K=m2("editorWidth",null),O=U2.current||280;z2.current=Number.isFinite(K)?K:O}if(V.style.setProperty("--editor-width",`${z2.current}px`),!w$.current){let K=m2("dockHeight",null);w$.current=Number.isFinite(K)?K:200}V.style.setProperty("--dock-height",`${w$.current}px`)},[Z0]),b(()=>{if(!y0||Z)return;let V=(K)=>{if(K.ctrlKey&&K.key==="`")K.preventDefault(),S$()};return document.addEventListener("keydown",V),()=>document.removeEventListener("keydown",V)},[S$,y0,Z]),b(()=>{if(Z)return;let V=(K)=>{if(K.ctrlKey&&K.shiftKey&&(K.key==="Z"||K.key==="z")){K.preventDefault(),m0();return}if(K.key==="Escape"&&l0)K.preventDefault(),u0()};return document.addEventListener("keydown",V),()=>document.removeEventListener("keydown",V)},[m0,u0,l0,Z]);let y5=Boolean(k_&&k_===(N_?.turn_id||U_));if(N)return B`
        ...
        `;return B`
        <div class=${`app-shell${s0?"":" workspace-collapsed"}${Z0?" editor-open":""}${Z?" chat-only":""}${l0?" zen-mode":""}`} ref=${r$}>
            ${!Z&&B`
                <${D9}
                    onFileSelect=${F2}
                    visible=${s0}
            ...
            ${O0&&B`
                <div class="editor-pane-container">
                    ...
                    ${Z0&&B`
                        ...
                        />
                    `}
                    ${Z0&&B`<div class="editor-pane-host" ref=${l$}></div>`}
                    ${Z0&&a_&&v0.has(a_)&&B`
                        <${y9}
            ...
            <div class="container">
                ...
                <${v6}
                    ...
                    onDeletePost=${q5}
                    onOpenWidget=${D5}
                    emptyMessage=${L?`No posts with #${L}`:F?`No results for "${F}"`:void 0}
                    agents=${R_}
                    user=${t_}
                    reverse=${_4}
                    removingPostIds=${h$}
                ...
                <${x8}
                    ...
                    onRemoveQueuedFollowup=${W5}
                    onSubmitIntercept=${L5}
                    onMessageResponse=${s2}
                    onSubmitError=${T}
                    onPopOutChat=${W?void 0:A5}
                    isAgentActive=${n2}
                    activeChatAgents=${J0}
                    currentChatJid=${j}
                    connectionStatus=${z}
...
//# debugId=07B1C2BBED5CC71F64756E2164756E21
