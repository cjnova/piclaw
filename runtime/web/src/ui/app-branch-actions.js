...
import { RENAME_BRANCH_FORM_GUARD_MS } from './app-shell-state.js';

/** Open the branch-rename form when no local/global submit lock is active. */
export function openRenameBranchForm(options) {
    const {
...
export function closeRenameBranchForm(options) {
 ...
 *
 * Keeps duplicate-submit locks in sync with both local refs and the shared window-global lock.
 */
export async function renameCurrentBranch(options) {
    const {
        hasWindow = typeof window !== 'undefined',
        currentBranchRecord,
        nextName,
...
export async function pruneCurrentBranch(options) {
    const {
        hasWindow = typeof window !== 'undefined',
        targetChatJid = null,
        currentChatJid,
...
export async function restoreBranch(options) {
    const {
        targetChatJid,
        restoreChatBranch,
        currentChatBranches = [],
...
export async function runBranchLoader(options) {
    const {
