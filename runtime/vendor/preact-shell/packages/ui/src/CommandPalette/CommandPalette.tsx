...
export interface CommandPaletteSource {
  search: (query: string) => CommandPaletteCommand[];
...
export function CommandPalette({ visible, onClose, source }: CommandPaletteProps) {
  const [query, setQuery] = useState("");
  const [selectedIndex, setSelectedIndex] = useState(0);
  const inputRef = useRef<HTMLInputElement>(null);
  const results = useMemo(() => source.search(query), [source, query]);
