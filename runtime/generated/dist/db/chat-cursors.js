 ...
 */
export function rollbackChatRunWithError(chatJid, failed) {
    const db = getDb();
    db.transaction(() => {
        db.prepare(`
    ...
    `).run(chatJid, failed.prevTs);
      ...
      SET cursor_ts           = ?,
          ...
          failed_created_at   = ?
      WHERE chat_jid = ?
    `).run(failed.prevTs, failed.prevTs, failed.failedTs, failed.messageId, failed.threadRootId ?? null, failed.createdAt, chatJid);
    })();
}
// ---------------------------------------------------------------------------
 ...
 */
export function getAgentReplyStateAfter(chatJid, afterTs) {
    const db = getDb();
    ...
    WHERE chat_jid = ?
      AND timestamp > ?
  `).get(chatJid, afterTs);
    if ((row?.has_terminal_bot ?? 0) > 0)
        return "terminal";
    if ((row?.has_any_bot ?? 0) > 0)
        return "partial";
    return "none";
}
/**
