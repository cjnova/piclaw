 ...
 */
export function snapshotSessionEntryCount(session) {
    const entries = session?.sessionManager?.getEntries?.();
    return Array.isArray(entries) ? entries.length : null;
}
export function inspectBlankTurnSessionDelta(session, baselineEntryCount) {
    if (!Number.isFinite(baselineEntryCount) || (baselineEntryCount ?? -1) < 0)
        return null;
    const entries = session?.sessionManager?.getEntries?.();
    ...
    for (const entry of appended) {
        ...
        if (role === "user")
            appendedUserMessageCount += 1;
        if (role === "assistant")
            appendedAssistantMessageCount += 1;
        if (role === "toolResult" || role === "tool_result")
            appendedToolResultMessageCount += 1;
    }
...
export function isBlankTurnSessionDelta(delta) {
    if (!delta)
        return false;
    if (delta.appendedEntryCount <= 0)
        return false;
    if (delta.appendedUserMessageCount <= 0)
        return false;
    if (delta.appendedAssistantMessageCount > 0)
        return false;
    if (delta.appendedToolResultMessageCount > 0)
        return false;
    return true;
}
