...
function appendSenderFilter(conditions, params, senderFilter, qualifier = "") {
    if (!senderFilter)
        return;
...
function buildContentExcerpt(content, terms, excerptChars) {
    if (!terms.length || excerptChars <= 0)
        return null;
...
function parseContentLines(input) {
    const trimmed = input?.trim();
    if (!trimmed)
        return null;
...
function getPatternInput(params) {
...
function lineMatchesPattern(line, pattern, regex) {
    if (!pattern)
        return false;
    if (!regex)
...
function collectPatternMatches(content, pattern, regex, captureGroup = 0) {
    if (!pattern)
        return [];
    if (!regex) {
        if (captureGroup > 0)
...
function buildLineView(content, range, grep, detailsMaxChars) {
    ...
    for (let lineNumber = selectedStart; lineNumber <= selectedEnd; lineNumber += 1) {
        ...
        if (grepNeedle && !line.toLowerCase().includes(grepNeedle))
            continue;
...
function listMessageWindow(chatJid, roleFilter, senderFilter, limit, offset, afterTs, beforeTs, afterRow, beforeRow) {
    const db = getDb();
    const conditions = [];
    const params = [];
    if (chatJid) {
...
function buildGrepLineView(content, pattern, regex, contextLines, detailsMaxChars) {
    const rawLines = content.split(/\r?\n/);
...
function runSearch(query, chatJid, roleFilter, senderFilter, limit, offset, afterTs, beforeTs, afterRow, beforeRow) {
    const db = getDb();
    const trimmed = query.trim();
    if (!trimmed)
        return [];
    ...
    try {
        ...
        params.push(...timeValues);
        for (const c of rowClauses)
            conditions.push(c.replace(/\browid\b/g, "messages.rowid"));
        params.push(...rowValues);
...
function fetchByRowId(chatJid, roleFilter, senderFilter, rowId) {
    const db = getDb();
...
function fetchContextRows(chatJid, rowId, roleFilter, senderFilter, before, after) {
    const db = getDb();
    const beforeConditions = ["chat_jid = ?", "rowid < ?"];
    const beforeParams = [chatJid, rowId];
    if (roleFilter !== null) {
    ...
    const afterParams = [chatJid, rowId];
    if (roleFilter !== null) {
    ...
    const beforeRows = before > 0 ? db.prepare(beforeSql).all(...beforeParams, before) : [];
    beforeRows.reverse();
    const afterRows = after > 0 ? db.prepare(afterSql).all(...afterParams, after) : [];
...
function executeGet(params, defaultChat) {
    ...
    for (const rowId of rowIds) {
        ...
        const { context_before, context_after } = fetchContextRows(row.chat_jid, row.rowid, roleFilter, senderFilter, contextBefore, contextAfter);
...
function executeGrep(params, defaultChat) {
...
function executeExtract(params, defaultChat) {
    const pattern = getPatternInput(params);
    if (!pattern) {
        return {
    ...
    if (sort === "asc")
        resultValues.sort((a, b) => a.value.localeCompare(b.value));
    if (sort === "desc")
        resultValues.sort((a, b) => b.value.localeCompare(a.value));
...
function executeDiff(params, defaultChat) {
...
export function runMessagesTool(params, defaultChat = "web:default", postFn) {
    ...
    if (action === "grep")
        return executeGrep(params, defaultChat);
    if (action === "extract")
        return executeExtract(params, defaultChat);
    if (action === "diff")
        return executeDiff(params, defaultChat);
    if (action === "add")
